...
Recharge/cooldown audit after the rest ordering stop:
...
- Actor artifacts currently carry `actionDefinitionIds` and resource current/maximum values only; they do not carry recharge policy metadata. The C6 proof is domain-only and the ledger correctly records `monster action catalog recharge projection` as the remaining seam.
- Adding automatic recharge now would therefore require a deliberate portable authoring/policy binding and die-authority contract. Per the architecture charter, no monster/action-ID branch or inferred recharge policy was added.
...
Continue Family P without repeating final Zone or `damage.taken/dealt` evidence. Do not implement rest triggers until trigger-vs-rest-expiry ordering is deliberately defined, and do not productionize Recharge until a portable policy/die-authority binding exists. Audit the next already-determined authoritative event family, starting with existing turn/round lifecycle producers and their effect/frequency composition points; implement only if the active actor, trigger ordering, frequency ownership, and lifetime semantics are already explicit in current contracts. Otherwise record the semantic gap and move to another already-owned event producer. Keep Family P `INCOMPLETE` until its complete required event/frequency matrix is evidenced.