...
Do not reopen Families S/T/U or repeat the now-proven Family V recast replacement. Families V/W/X/Y now have explicit authority or durable-source ownership blockers and should not be guessed. Move to the next non-blocked ledger audit, Family Z (`spellcasting-meta`): reconcile the existing typed component/casting/slot runtime, unified initiative/freeform spell execution, installed Common Play spell lowering, durable material/item payment boundary, and current spell identity lookup against the row's complete required semantics. Reuse existing spell/runtime/payment owners; if slot/access/material-source ownership or named identity dispatch remains a real blocker, preserve Z as `INCOMPLETE` and continue to the next non-blocked family rather than adding another spell engine.
