...
탁자에서 DM은 도감 → 아이템 탭 위쪽 "캠페인 아이템"에 이 JSON을 붙여넣어 캠페인에 저장한다. 같은 파서로 검사해 경고를
보여 주고, 저장한 아이템은 모든 자리의 아이템 추가 창·도감에 뜬다. "주기"로 캐릭터에게 주면 가방에는 아이템 id만
남으므로, DM이 "고치기"로 수치를 바꾸면 가진 캐릭터 모두의 시트에 바로 반영된다. "미식별로 주기"를 켜고 주면 가방에는
"미식별 반지" 같은 이름만 보이고 효과·충전이 꺼져 있다가, 가방 줄의 "식별" 버튼(식별 주문·짧은 휴식 — DM 판정)이나
조율로 드러난다 (D364).

## 6.6 은혜(특성)로 주기 (D365)

같은 JSON을 물건이 아니라 캐릭터의 특성으로 줄 수 있다 — 캠페인 보상, 축복, 악마와의 계약 같은 것. 붙여넣기 창의
"은혜(특성)로 주기" 버튼이나 캠페인 아이템의 "은혜(특성)로 주기" 체크로 준다. 가방에는 없고 시트의 "은혜·보상" 칸에
뜨며, 조율·착용 없이 늘 작동한다(`attunement`는 무시). `contract`의 버튼, `uses` 풀, 저항·보너스가 아이템과 똑같이
돌고, `notes`는 그 줄에 DM 판정으로 보인다. 줄의 ✕로 거둔다.
