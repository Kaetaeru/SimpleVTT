...
| `use` | 객체 | 턴 패널에서 썼을 때: `healing`("2d4+2" 형식, 마실 대상을 골라 굴려 회복)과 `consumes`(true면 하나 줄어듦). 회복이 있으면 소모된다. 이름에 "물약"이 들어가도 `use`가 없으면 회복하지 않는다(D247) |
...
- 충전(charges)과 아이템에서 주문 시전은 아직 계산하지 않는다. `notes`에 적는다.
- `damageDice`는 무기의 피해 타입으로 굴린다. 다른 타입의 추가 피해(예: 화염 1d6)는 `notes`에 적는다.
