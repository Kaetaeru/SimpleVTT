...
| `C1-08` | _pending run_ | `scripts/run-tauri-e2e-c1-mp.mjs` (launcher switch `-C1MP`, workflow `.github/workflows/c1-clean-play-multiplayer-windows.yml`). Steps C1-MP-01 … C1-MP-09; the evidence artifact `C1-MP-WIN-<sha>` carries `c1-mp-summary.json` and a screenshot of every instance after every step. |
