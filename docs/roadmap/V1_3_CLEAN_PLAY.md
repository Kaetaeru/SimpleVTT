...
| `C1-06` | Spells, wave 3 (2024 rules): the combat-adjacent spells the C1-03 review left tracked because the engine lacked a kind — revive, HP maximum, dispel, Death Ward, the Mage Armor AC formula, spent-on-hit smites, retaliation (Fire Shield) and summons that add catalog creatures to the scene | engine + data | kernel kinds `revive`/`maximum-hp`/`dispel`; tracked-effect `preventsDeath`/`retaliation`/`armorClass.addTargetSaveModifier`/`attackDamage.consumeOnUse`; definition `summons`; domain suite `spellAuthoredWave3`, app suite `spellSummon` |
| `C1-07` | Monsters, finished: "A 또는 B를 조합해 N번" multiattack lines become routines the DM resolves as one action; Legendary Resistance re-judges a stat-block saving-throw action (breath, gaze) the same way it re-judges a spell | engine | generator parses the choice lines; `resolveMultiattackRoutine` covers them; the atomic saving-throw adapter honours `forcedSaveSuccess`; tests on a choice routine and on a legendary target of a breath weapon |
| `C1-08` | Real multiplayer acceptance of the clean-play path: three Windows Tauri instances (DM Host without a character, two players) over the production TCP transport — SRD monster from the 인카운터 search, a player 물러남 routed to the Host, Initiative from the DM tools, a player spell and a player weapon attack, the monster multiattack routine, a client restart with catch-up, and the explicit session end | acceptance | `scripts/run-tauri-e2e-c1-mp.mjs` passes in the `C1 Clean Play Multiplayer Windows Verification` workflow; the run URL and its evidence artifact are recorded |
| `C1-09` | A written theater-of-mind one-shot ("잿빛 관문의 늑대들", 2024 rules: social check, ambush with a wolf pack in darkness, engagement and 물러남/기회공격, a party long rest with campaign time and rations, a hidden roll disclosed later, a multiattack boss, healing, XP, session end) played end to end on Windows H+P1+P2 through the real UI; defects the play surfaces are fixed | acceptance | `scripts/run-tauri-e2e-tom-scenario.mjs` passes in `Theater-of-Mind Scenario Windows Verification`; the scenario document records the run and the findings |
...
| `C1-05` | `a00c54f53add8270fe8b953cb7d9746630ae3f8b` | PR #372. Walkthrough (DM preview and player preview, initiative mode, dev build after C1-04) — see "C1-05 walkthrough" below. Stale suites resolved: `campaignDmLibraryPcPresetRuntime` (materialization guard accepts the null-checked map) and `productionOfficialActions` (a spellbook ritual is offered as a ritual cast beside the prepared spells). |

| `C1-06` | `c5c0f12838f233d4ab1012ad4584f012a5528281` | PR #373. Coverage moves to **82 authored / 146 tracked**. Kernel: `revive` (healing op with `revive`, clears dead/unconscious/death saves), `maximum-hp` (the existing maximum-hp op plus matching healing), `dispel` (removes every `spell`-tagged effect on the target). Tracked effects: `preventsDeath` (the damage finalizer leaves 1 HP and ends the ward), `retaliation` (the attack compiler adds damage to the attacker on a hit; `effectRetaliations` feeds the production attack transaction, melee only when flagged), `armorClass.addTargetSaveModifier` (Mage Armor = 13 + DEX save modifier), `attackDamage.consumeOnUse` + `dice.dicePerSlotAboveBase` (Divine Smite, Shining Smite are spent by the first hit; the attack compiler removes the effect on success). Definition `summons {monsterId,count,countPerSlotAboveBase}`: `spellSummonRuntimeAdapter` instantiates the catalog creature on the caster's side after the cast commits (Animate Dead → Skeleton ×1 +2/slot, Create Undead → Ghoul ×3). Spells: Revivify, Raise Dead, Resurrection, True Resurrection, Reincarnate, Aid, Death Ward, Dispel Magic, Mage Armor, Freedom of Movement, Aura of Life, Earthquake, Divine Smite, Shining Smite, Animate Dead, Create Undead; Fire Shield gained its retaliation. Still tracked (146): utility/narrative spells and the movement, sense, wall, contest and summon-stat-block spells listed under C1-03. |

| `C1-07` | `1e1619189b99d0c357d7cbe826260ff51bc1adac` | PR #374. Multiattack routines cover **174/180** lines: the generator parses "A 또는 B를 조합해 N번" / "A 와 B" / "A 또는 B를 사용해 N번" into one step with `alternatives` (the first resolved attack is the default; alternatives also resolve through the attack's English name, e.g. "Longbow"); the DM-tools label reads "시미터/단궁 2회". The six lines left are "여러 번" with no attack named, the aboleth's "두 번 하고 X 또는 Y를 사용" and the ancient gold dragon's "공격 또는 공격" translation slip; they keep the per-attack count. Legendary Resistance now re-judges a stat-block saving-throw action too: the atomic saving-throw adapter reads `forcedSaveSuccess` before its transaction, marks the target's preview as 성공 and the transaction judges that save against target 0 with a `legendary-resistance:auto-success` source; the timing adapter drives the staged card to completion before annotating it. Tests: `srdMonsterChoiceRoutine`, `srdMonsterLegendaryBreath` (adult red dragon breath vs. aboleth: 실패 → 전설 저항 → 성공, half damage, 2/3). Pre-existing, not in CI: `tests/domain/c9FamilyJExtraAttackPayment` ("rejected outside the action attack bucket") fails on the base branch and is unrelated. |
| `C1-08` | `7d6c9b380b31948f1d5002d8b6a52c471575797d` | PR #379. `scripts/run-tauri-e2e-c1-mp.mjs` (launcher `-C1MP`, workflow `C1 Clean Play Multiplayer Windows Verification`) passed on the PR head `555a312c2dd32ff21abf6cc586a16676547c41a9`: run https://github.com/Kaetaeru/SimpleVTT/actions/runs/34003720842, artifact `C1-MP-WIN-555a312c…`; `docs/design/ui-ux/v1_3/c1-08/` keeps `c1-mp-summary.json` and one screenshot per step. Steps: C1-MP-01 (the Host scene holds only the two joined players; no Aelar in the workspace or 기록) · 02 (죽음의 개 from the 인카운터 search reaches both players with AC 10 / HP 39) · 04 (이니셔티브 시작 from the DM tools; order, round and current actor converge) · 03 (P1's 물러남 from the dock reaches the Host and P2) · 05 (Guiding Bolt: one Host resolution, HP and the level-1 slot converge, 2 → 1) · 06 (P2's 대검) · 07 (다중공격 · 물기 2회 from the 인카운터 pane, both attacks on every peer) · 08 (P2 killed and relaunched; rejoin catch-up exactly once) · 09 (세션 종료 from the 세션 screen; both players offline, no silent reconnect). The same head re-ran the whole Windows acceptance matrix (W1–W4-07, W6-08, W7-04/05/08, W8-02 four-peer, W9-02 families C/D/H, W9-03, X1-03, storage, golden journey): 43 checks green. Found and fixed by the run: the reference fixture 기록 (Aelar/Mira) leaked into hosted sessions; players saw "원격 Resolution 적용 · <id>", "Host event #N" and raw actor ids instead of the Host's wording; spell results read "<id> takes N radiant damage"; a join racing another Campaign write was rejected with a stale library generation. |
| `C1-09` | `4db4e88fca887bcbbec3fd6af571a03f0e2828b3` | PR #381. `docs/design/scenarios/ashen-gate-wolves.md` (scenario, VTT mapping, run results, findings) with evidence under `docs/design/scenarios/evidence/ashen-gate-wolves/`; run https://github.com/Kaetaeru/SimpleVTT/actions/runs/34028996395. Fixed on the way: engagements from a melee resolution now ride on the resolution event; a completed result card no longer defers a player's declaration, loses the DM's chosen opportunity attack or locks 이니셔티브 시작/종료; the Long Rest commit broadcasts the campaign clock and rations; the owner's write-back refreshes the Host's projection (wounded players can be offered a rest) and a rejected offer is shown; a Campaign that moved on since the offer no longer drops the approval; "내성 성공 · 피해 없음" wording; disclosures name the action; 이니셔티브 종료 clears the fight's declarations. Open: per-player long-rest offers each apply +8h/rations (party rest should apply once); the movement row is Initiative-only. |
