...
| `T2-03` | 커널 | 스냅샷 저장·재개, 새 피어 id 재접속 후 카드 답변, write-back(HP·임시 HP·자원; 제품 경로의 캐릭터 라이브러리까지, 솔로·호스트·클라이언트), 몬스터 정의 비공개·HP 단계(D22), 귓속말, 원장 무결 | write-back 나머지(슬롯·히트 다이스·죽음 내성), Windows H+P1+P2 |
