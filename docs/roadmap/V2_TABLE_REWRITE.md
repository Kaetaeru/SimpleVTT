...
| `T2-04` | 슬라이스 A | DM/플레이어 워크스페이스 뼈대 (`src/table/ui/`): 상단 바·테이블(토큰 카드, 초점 카드, D42 판정 메뉴, 질문)·사이드바 7탭·명령 센터(대상 지정)·재량 바·토큰 HUD·Ctrl+K·시트 서랍. 증거 [T2-04.md](evidence/T2-04.md) | 준비실·호스트 라이브러리·장면 묶음, 아이템 지급·토큰 드롭, 드래그, 소유자 검토, Windows |
| `T2-07` | 미착수 | — | 전환 |
...
| `T2-04` 슬라이스 A | 이 브랜치 head | `tests/table/workspace.test.ts` 3건 (`npm run test:workspace`, 정적 렌더 + 모델), [evidence/T2-04.md](evidence/T2-04.md)의 Chromium 캡처 9장 (dev 미리보기, Linux) |
