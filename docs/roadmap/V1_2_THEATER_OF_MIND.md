...
| `T1-04` | `0fed699d1bf0cd3c5efb345a5277c1d3749667e6` (PR #361) | `encounterGroupRuntimeAdapter`: `instantiateCombatantGroup` shares one initiative roll (same d20 face replayed through the normal path), group/ungroup by hand, groups follow their members; boards fold groups into one card with one-tap area targeting; Encounter pane ×N 한 무리; `tests/ui/encounterGroups.test.ts`. |
| `T1-05` | `60acb9e06780d97660006f2421e5363b1e10f0c4` (PR #362) | `movementDeclarationRuntimeAdapter`: 접근/물러남/그대로 declarations, 물러남 raises one DM prompt (`scene.pendingWithdrawal`) only when an engaged hostile still has its reaction and a melee attack; `answerWithdrawalPrompt` resolves the opportunity attack through the manual movement-reaction path; 이탈 suppresses it; the per-card 기회공격 유발 button is gone; dock buttons act on the current-turn creature; `tests/ui/movementDeclarations.test.ts`. |
| `T1-06` | `f289b4c87a32e2688ad3a66fe649daf7d9a4f6fc` (PR #363) | Resolution layer auto-advances every stage (interrupt and ability-check DC still wait); `resolutionPostHocRuntimeAdapter.applyPostHocToggle` re-judges a committed attack with the same first d20 (이점/불리점/판정 그대로/엄폐 ½/¾), 피해 절반 via healing-correction, 닿지 않음 undo + approach; roll-state filter and target-AC contributor registries; `tests/ui/resolutionPostHocToggles.test.ts`. |
| `T1-07` | `7668c1f8397d9fc617010107543bcc2b04ff7ad1` (PR #364) | `sceneConditionRuntimeAdapter`: 어둠/안개 scene chips, 숨음/투명/엄폐 ½/엄폐 ¾ badges as public statuses read by the attack roll-state and target-AC hooks, narrative HP/condition edits with activity lines; Encounter pane 장면 chips and per-row 편집 fold; distance tool removed from `ProductionSessionWorkspaceBridge`, `ProductionSessionLifecycleBridge.tsx` and `MovementReactionBridge.tsx` deleted; `tests/ui/sceneConditionsBadges.test.ts`. |
