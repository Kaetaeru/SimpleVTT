...
| `S1-04` | `fde43db305eeba7c0d97dc735bb84abbbd80ee4d` | PR #387. Scenario 2 "무너진 종탑" (`docs/design/scenarios/fallen-bell-tower.md`, runner `scripts/run-tauri-e2e-tom2-scenario.mjs`, launcher `-TOM2`, workflow `Fallen Bell Tower Scenario Windows Verification`): `Fallen Bell Tower Scenario Windows Verification` run 34056344405 green on H+P1+P2 (장면1-마을 · 장면2-계단 · 막간-되돌리기 · 장면3-종루 · 마무리); Play Matrix, C1-MP and scenario 1 green on the same head. Ten defects surfaced and fixed (owner slot seeding, displayed AC with effects, status effects event-native in 자유 진행, Host and replica runtime carry-over across 이니셔티브 시작/종료, late-added combatants on replicas, spell tiles vs turn economy, kernel-rejected casts as refusals, narrative damage life state, immediately-complete check ending effects, reactions off-turn, silent guards in 안정화/붙잡기/되돌리기) — listed in the scenario document's 구동 결과. Evidence: `docs/design/scenarios/evidence/fallen-bell-tower/`. |
