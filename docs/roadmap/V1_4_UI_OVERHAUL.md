...
| `U1-02` | `6ab275c0e8e2b6c983ffdaf53fa437f1c63c1cf2` | PR #377. Product screens on the page-head pattern: the home dashboard (U1-01), 캐릭터 list (flat cards, portrait tile, actions row), 캠페인 (empty state, new-campaign editor, recent list), 콘텐츠, 규칙 (three-pane catalog), 설정 and the session start screen share one head (kicker, title, muted description, rule) and the token surfaces. Screenshots: `docs/design/ui-ux/v1_4/u1-02/{characters,campaign-empty,campaign-new,rules,settings,session-start}.png`. |
| `U1-04` | `6ab275c0e8e2b6c983ffdaf53fa437f1c63c1cf2` | PR #377. Creation wizard (stepper / form / summary), level-up (class grid, before → after summary) and the SimpleVTT sheet render flat on the tokens with sans numerals; the official paper sheet keeps its serif and paper look by design. Screenshots: `docs/design/ui-ux/v1_4/u1-04/{create,levelup,sheet}.png`. |
| `U1-05` | `6ab275c0e8e2b6c983ffdaf53fa437f1c63c1cf2` | PR #377. 1024: the session chrome scrolls sideways instead of wrapping and the rail narrows to 224 px; 768: the rail collapses to horizontal rows above and below the stage; 375: the home stacks to one column and the player session keeps the dock usable. The first-run choice sits on the tokens. Screenshots: `docs/design/ui-ux/v1_4/u1-05/{home-1024,dm-1024,player-768,home-375,player-375,first-run}.png`. |
| `U1-06` | `6ab275c0e8e2b6c983ffdaf53fa437f1c63c1cf2` | PR #377. Walkthrough (headless screenshots of every screen on the product and play paths at 1440×900 plus the widths above) after U1-01/U1-03 merged. Fixed on the way: the home kicker the Windows e2e harness waits for is kept; rail labels read 상대/아군; the connection badge and chrome tabs never wrap; the creation preview portrait block is flat. Left as is, with the reason: the official paper sheet keeps its serif (deliberate style); the reference preview fighter is named "Second Wind" and its activity carries a duplicate "이니셔티브 시작" entry (pinned mock data, not a production surface). Full UI suite on the U1-03 head: 1527 pass, 0 fail. |
