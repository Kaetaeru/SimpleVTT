...
The atomic saving-throw adapter's Fighter Indomitable lookup is also removed on the C8 convergence branch. Atomic replay consumes authoritative save facts, while the generic d20 follow-up now owns reroll, authored flat bonus, mandatory replacement result, resource payment, connected replay, and Undo. The named Indomitable app/domain paths are deleted.
...
The runtime-presence spell router and `legacySpellRuntimeHandler.ts` are deleted. Supported freeform and initiative spells now create or reuse the same authoritative TurnRuntime state and share event-native Undo. `productionSpellRuntimeAdapter.ts` and `phase09AuthoritativeSpellcastingAdapter.ts` now read the generated normalized spell definition catalog: spell identity locates authored data only, while generic definition structure selects the algorithm. They are therefore no longer named spell-execution seams. The earlier `spellcastingRuntimeAdapter.ts` action-ID compatibility table remains inventoried above until its projection compatibility path is removed.

Dash execution no longer recognizes `action.dash`. Any authored no-roll action with a positive `movementBudgetGainFeet` uses the same economy/event path. The duplicate Rogue Cunning Dash mutation/event branch is deleted; its structural clone now records Bonus Action and movement through the shared event. Standard no-roll status changes, check-threshold status changes, and declarative completion text now execute from `ActionVm` facts without recognizing standard-action IDs. Unknown action-ID proofs cover both paths. Ready configuration/routing and localized status consumers remain named seams.
...
The Bardic Inspiration grant and attack follow-up no longer own named execution paths. Any authored action with `runtimeEffectGrant` commits its declared economy, resource, duration, public Effect, connected replay, and Undo through the generic Resolver path; an unknown action ID proves the grant algorithm is identity-invariant. Effects that structurally declare a consumable failed-d20 add-die rule use the generic follow-up transaction and preserve connected event replay and Undo. Bardic Inspiration's class-level resource projection remains legacy.
...
Peerless Skill no longer owns a College-of-Lore runtime or domain Resolver. SRD 5.2.1 authors it as a failed ability-check/attack-roll additional Bardic Inspiration die whose resource payment occurs only if the modified roll succeeds. The generic d20 transaction now proves both families, miss-to-hit continuation without action mutation, failure-time resource preservation, connected owner persistence/reconnect/Undo, public provenance, and unknown-ID execution. The obsolete named-domain-output test was deleted.

Indomitable no longer owns a Fighter runtime or domain Resolver. SRD 5.2.1 p.48 defines a failed-save reroll with a Fighter-level bonus, mandatory new result, Long-Rest resource payment, and level-scaled uses. The generic d20 follow-up reuses the existing Resolver reroll operation, preserves authoritative save modifiers and roll state, and proves production owner prompting, Host/Client convergence, replay rejection, persistence, Undo, and unknown-ID execution. The obsolete named-domain-output tests were deleted.
...
- Cutting Words: corresponding Lore UI tests + `bardCollegeLore` domain tests. Peerless Skill coverage now comes from its SRD-derived generic production tests.
