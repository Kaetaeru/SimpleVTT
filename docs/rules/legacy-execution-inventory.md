...
| `src/app/phase09AuthoritativeSpellcastingAdapter.ts` | known spell ID still selects a `SpellMechanicDefinition` | authoritative spell execution | `phase09AuthoritativeSpellcastingAdapter.test.ts`, spellcasting regressions | runtime state, slots, event-native Undo | normalized Common Play spell IR; delete spell-ID lookup |
...
| `bardCollegeLoreCuttingWordsFollowUpRuntimeAdapter.ts` | Lore roll interceptor | Cutting Words domain/UI regression references | connected responder/owner, reaction/resource, pending roll, retry/Undo | Gate A Interaction + payment + roll reduction/recalculation |
