...
| `rogueCoreRuntimeAdapter.ts` | remaining Rogue core projection, Cunning Disengage, and Uncanny Dodge | `test:rogue-core` + connected Rogue regressions | turn economy, effect, reaction | generic entry point + economy + effect/interceptor |
| `bardicInspirationRuntimeAdapter.ts` (transitively installed), `bardicInspirationActionRuntimeAdapter.ts` | Bardic Inspiration resource/grant | Bardic Inspiration domain/UI tests | source/owner resource, target-held die | generic resource/content grant + Interaction |
...
The Bardic Inspiration attack follow-up no longer owns a named execution path. Effects that structurally declare a consumable failed-d20 add-die rule use one generic Resolver transaction, carry the resulting modifier into the canonical atomic attack, and preserve connected event replay and Undo. An external effect with renamed source, instance, tag, and label proves production identity invariance; Bardic Inspiration resource/grant projection remains legacy.

`rogueCunningHideEventRuntimeAdapter.ts` is replaced by the identity-agnostic `abilityCheckStatusEffectEventRuntimeAdapter.ts`. Any authored ability-check action with a threshold status effect now commits d20, economy, Effect, connected replay, attack-ending cleanup, and Undo evidence through the same path. A completely unknown action ID proves the generic production contract. Rogue action projection remains legacy.
