(() => {
  ...
  function applyScenario(id) {
    ...
    const retainedViewport = s.viewport || state.viewport || 'normal';
    const retainedReducedMotion = state.reducedMotion;
    Object.assign(state, BASE, s, {
      ...
      noCharacter: !!s.noCharacter,
      snapshotNotice: !!s.snapshotNotice,
      previewRoll: s.previewRoll || null,
      controlMode: !!s.controlMode,
      showCorrection: !!s.showCorrection,
      reducedMotion: retainedReducedMotion
    });
    state.liveSession = s.surface === 'play';
    syncControls();
    render();
    if (state.previewRoll && state.surface === 'sheet') {
      setTimeout(() => runStandaloneRoll(state.previewRoll), 80);
    }
  }
  ...
  function renderSurface() {
    switch (state.surface) {
      ...
      case 'session': return renderSessionLanding();
  ...
  function renderTutorial() {
    return shell('home',`<main class="tutorial-page" data-proto-id="PROTO-SURF-FIRST-RUN">
      ...
      <section class="tutorial-window" role="dialog" aria-modal="true" aria-label="SimpleVTT Tutorial">
        <div class="tutorial-top"><span class="review-kicker">WELCOME TO SIMPLEVTT</span><h1>테이블 위 D&D를 더 빠르고 명확하게.</h1><p>SimpleVTT는 배틀맵 VTT가 아닙니다. 캐릭터 시트와 판정 자동화에 집중한 Standalone 도구이면서, 필요할 때 Host/Join으로 같은 정보를 공유하는 Connected Session 도구입니다.</p></div>
        <div class="tutorial-body">
          <div class="tutorial-mode-row"><article class="tutorial-mode"><strong>Standalone Character</strong><p>실제 테이블에서 노트북 하나로 Character Sheet, 능력·공격·피해·일반 주사위, 자원과 상태를 빠르게 사용합니다.</p></article><article class="tutorial-mode"><strong>Connected Session</strong><p>Host는 DM, Client는 Player입니다. Actor·행동·판정·주사위·기록을 공유하지만 Core에는 battlemap이나 token position이 없습니다.</p></article></div>
          <div class="tutorial-section-title">처음 사용할 Character Sheet 스타일</div>
          <div class="sheet-choice-grid"><button class="sheet-choice ${state.tutorialChoice==='official'?'selected':''}" data-tutorial-sheet="official"><span class="choice-check">${state.tutorialChoice==='official'?'SELECTED':''}</span><strong>Official-style Sheet</strong><p>종이 Character Sheet의 정보 구조를 따라 전체 기록을 밀도 있게 읽는 방식. SimpleVTT 전용 렌더링으로 제공합니다.</p></button><button class="sheet-choice ${state.tutorialChoice==='simplevtt'?'selected':''}" data-tutorial-sheet="simplevtt"><span class="choice-check">${state.tutorialChoice==='simplevtt'?'SELECTED':''}</span><strong>SimpleVTT Sheet</strong><p>현재 상태와 직접 사용할 Roll/Action을 빠르게 찾도록 재배치한 디지털 최적화 방식입니다.</p></button></div>
          <div class="tutorial-section-title">어디서 시작할까요?</div>
          <div class="tutorial-orientation"><div class="orientation-card"><strong>캐릭터</strong><span>Create / Import / Library / Edit / Level Up</span></div><div class="orientation-card"><strong>Host Session</strong><span>열면 바로 live DM Freeform으로 들어갑니다.</span></div><div class="orientation-card"><strong>Join Session</strong><span>로컬 Character를 선택해 이미 진행 중인 Session에 참가합니다.</span></div></div>
  ...
  function renderHome() {
    ...
    return shell('home',`<main class="product-page" data-proto-id="PROTO-SURF-HOME">
      <div class="home-hero"><section class="hero-card"><span class="review-kicker">TABLETOP COMPANION</span><h2>오늘의 플레이를 시작하세요.</h2><p>Character Sheet를 단독으로 사용하거나, DM이 Session을 열고 Player가 자신의 Character로 참가할 수 있습니다. Core Play는 Actor와 판정 중심이며 battlemap을 요구하지 않습니다.</p><div class="hero-actions"><button class="btn primary" data-nav="characters">캐릭터 열기</button><button class="btn" data-action="new-character">새 캐릭터</button><button class="btn quiet" data-action="open-tutorial">Tutorial 다시 보기</button></div></section><div class="session-choice"><section class="session-entry-card"><div><span class="badge dm">HOST = DM</span><h3>Host Session</h3><p>Session을 열면 Lobby/Ready 없이 즉시 live Freeform Play가 시작됩니다. Player 0명도 정상입니다.</p></div><button class="btn primary" data-nav="host">Host Session</button></section><section class="session-entry-card"><div><span class="badge ally">CLIENT = PLAYER</span><h3>Join Session</h3><p>Host 연결 정보와 자신의 로컬 Character를 선택해 진행 중인 Session에 참가합니다.</p></div><button class="btn primary" data-nav="join">Join Session</button></section></div></div>
      <div class="page-heading"><div><h2>최근 Character</h2><p>마지막으로 사용한 Character를 바로 열 수 있습니다.</p></div><button class="btn quiet" data-nav="characters">전체 보기</button></div><div class="library-grid">${renderCharacterCard(c)}${renderCharacterCard(F.characters[1])}${renderCharacterCard(F.characters[2])}</div>
    </main>`);
  }
  ...
  function sheetBlock(title,rows) { return `<section class="sheet-block"><h3>${esc(title)}</h3>${rows.join('')}</section>`; }
  function sheetRollRow(item) { return `<div class="sheet-row"><span>${esc(item.label)}</span><strong>${esc(item.value)}</strong><button class="roll-btn" data-sheet-roll="${esc(item.id)}">ROLL</button></div>`; }

  function renderSheet() {
    const c = currentCharacter();
    return `<section class="sheet-root" data-proto-id="${state.sheetStyle==='official'?'PROTO-SURF-CHAR-SHEET-OFFICIAL':'PROTO-SURF-CHAR-SHEET-SVTT'}"><div class="sheet-toolbar"><button class="btn quiet" data-nav="characters">← Library</button><strong>${esc(c.name)} · ${esc(c.classLine)}</strong><span class="grow"></span><button class="btn ${state.sheetStyle==='official'?'primary':''}" data-sheet-style="official">Official</button><button class="btn ${state.sheetStyle==='simplevtt'?'primary':''}" data-sheet-style="simplevtt">SimpleVTT</button><button class="btn" data-action="edit-character">Edit</button><button class="btn" data-action="level-up">Level Up</button></div><div class="sheet-workspace">${state.sheetStyle==='official'?renderOfficialSheet(c):renderSimpleSheet(c)}</div></section>`;
  }
  ...
  function renderSessionLanding() {
    return shell('session',`<main class="product-page"><div class="page-heading"><div><h2>세션</h2><p>Host와 Join은 서로 다른 첫-class 경로입니다. 어느 쪽도 Lobby/Ready를 기본 lifecycle로 사용하지 않습니다.</p></div></div><div class="entry-layout"><section class="entry-card"><div><span class="badge dm">HOST = DM</span><h3>Host Session</h3><p>새 Session을 열고 즉시 live Freeform DM workspace로 들어갑니다. Player 0명도 정상입니다.</p></div><button class="btn primary" data-nav="host">Host Setup</button></section><section class="entry-card"><div><span class="badge ally">CLIENT = PLAYER</span><h3>Join Session</h3><p>Host 연결 정보와 local Character를 선택해 이미 진행 중인 live Session에 참가합니다.</p></div><button class="btn primary" data-nav="join">Join Setup</button></section></div></main>`);
  }
  ...
  function actorEligibility(id) { if (!state.action) return null; return F.targetEligibility[state.action]?.[id] || null; }
  ...
  function renderHandout() {
    ...
    if (state.handoutLocalDismissed && state.handout === 'overlay') {
      return `<button class="btn" data-action="reopen-handout" style="position:absolute;z-index:12;right:14px;bottom:14px;pointer-events:auto">Handout 다시 열기</button>`;
    }
  ...
  function visibleCapabilities() { if (state.hotbarPage === 'Mixed') return F.capabilities; return F.capabilities.filter(c => c.page === state.hotbarPage || (state.hotbarPage === 'Custom' && c.page === 'Custom')); }
  ...
  function renderUtility() {
    ...
    if (state.utility==='quick-sheet') { title='Quick Sheet'; const c=currentCharacter(); body=`<div class="utility-list"><div class="utility-row"><strong>${esc(c.name)}</strong><span>${esc(c.classLine)} · HP ${c.hp}/${c.hpMax}${c.tempHp?` +${c.tempHp} Temp`:''}</span></div><div class="utility-row"><strong>AC / Speed</strong><span>${c.ac} · ${esc(c.speed)}</span></div><div class="utility-row"><strong>Conditions</strong><span>${(c.conditions||[]).join(', ')||'None'}</span></div><button class="btn" data-action="open-full-sheet">Open Full Sheet</button></div>`; }
    if (state.utility==='rules') { title='Rules Lookup'; body=`<label class="form-field">Search<input value="grapple"></label><button class="btn primary" style="margin-top:6px">Search</button><div class="utility-list" style="margin-top:8px"><div class="utility-row"><strong>Grappling</strong><span>authoritative catalog fixture</span></div><div class="utility-row"><strong>Escaping a Grapple</strong><span>related fixture result</span></div></div>`; }
  ...
  function runStandaloneRoll(id) {
    ...
    sheetSettleTimer = setTimeout(()=>{ const flight=sheetDiceLayer?.querySelector('.dice-flight'); flight?.classList.add('settled'); const result=sheetDiceLayer?.querySelector('.sheet-roll-result'); if(result)result.innerHTML=`<strong>${esc(r.label)} · ${esc(r.result)}</strong><span>${esc(r.notation)} · ${esc(r.detail)}</span>`; },reduced?20:1020);
  ...
  function actorClick(id) {
    ...
    if (state.action) {
      ...
      if (c?.targetMode === 'multi' && e?.valid) { state.selectedTargets = state.selectedTargets.includes(id) ? state.selectedTargets.filter(x=>x!==id) : [...state.selectedTargets,id]; state.notice=null; render(); return; }
    }
  ...
  function capabilityClick(id,el) {
    ...
    if (c.targetMode === 'self' || c.targetMode === 'none') { state.action=id; state.selectedTargets=[]; state.notice=`${c.label} · fixture ${c.targetMode} capability selected.`; render(); return; }
  ...
  function startConnectedResolution(actionId,targetIds) {
    ...
    render();
    connectedTimerA=setTimeout(()=>{ state.resolution='dice'; state.connectedRoll='attack'; state.notice=null; render(); },state.reducedMotion?120:650);
    connectedTimerB=setTimeout(()=>{ state.resolution='result'; render(); },state.reducedMotion?500:1850);
    connectedTimerC=setTimeout(()=>{ state.resolution=null; state.connectedRoll=null; state.action=null; state.selectedTargets=[]; render(); },state.reducedMotion?2200:3900);
  }

  function runResponseDice(kind) {
    [connectedTimerA,connectedTimerB].forEach(t=>t&&clearTimeout(t));
    state.resolution='dice';
    state.connectedRoll=kind==='concentration'?'concentration':'attack';
    render();
    connectedTimerA=setTimeout(()=>{state.resolution='result';render();},state.reducedMotion?350:1250);
    connectedTimerB=setTimeout(()=>{state.resolution=null;state.connectedRoll=null;render();},state.reducedMotion?1900:2900);
  }
  ...
  function handleNav(dest) {
    ...
    else if (dest==='session') { state.surface='session'; state.view='offline'; }
    else if (dest==='host') { state.surface='host'; state.view='offline'; }
    else if (dest==='join') { state.surface='join'; state.view='offline'; state.noCharacter=false; }
  ...
  root.addEventListener('click',event=>{
    ...
    const nav=event.target.closest('[data-nav]'); if(nav){handleNav(nav.dataset.nav);return;}
    const openCharacter=event.target.closest('[data-open-character]'); if(openCharacter){state.selectedCharacterId=openCharacter.dataset.openCharacter;state.surface='sheet';state.view='offline';render();return;}
    const sheetStyle=event.target.closest('[data-sheet-style]'); if(sheetStyle){state.sheetStyle=sheetStyle.dataset.sheetStyle;render();return;}
    const tutorialSheet=event.target.closest('[data-tutorial-sheet]'); if(tutorialSheet){state.tutorialChoice=tutorialSheet.dataset.tutorialSheet;render();return;}
    const actorEl=event.target.closest('[data-actor]'); if(actorEl){actorClick(actorEl.dataset.actor);return;}
    const capEl=event.target.closest('[data-capability]'); if(capEl){capabilityClick(capEl.dataset.capability,capEl);return;}
    const page=event.target.closest('[data-hotbar-page]'); if(page){state.hotbarPage=page.dataset.hotbarPage;render();return;}
    const util=event.target.closest('[data-utility]'); if(util){const u=util.dataset.utility;state.utility=u==='none'||state.utility===u?null:u;render();return;}
    const filter=event.target.closest('[data-activity-filter]'); if(filter){state.activityFilter=filter.dataset.activityFilter;render();return;}
    const jump=event.target.closest('[data-review-jump]'); if(jump){applyScenario(jump.dataset.reviewJump);return;}
    const context=event.target.closest('[data-context]'); if(context){if(context.dataset.context==='control'&&state.contextMenu)state.controlActorId=state.contextMenu.actorId;state.contextMenu=null;render();return;}
    ...
    switch(action.dataset.action){
      ...
      case 'resolve-response': runResponseDice(state.resolution);break;
      case 'dismiss-handout': state.handoutLocalDismissed=true;render();break;
      case 'reopen-handout': state.handoutLocalDismissed=false;render();break;
      case 'open-full-sheet': state.fullSheet=true;render();break;
  ...
  root.addEventListener('contextmenu',event=>{const a=event.target.closest('[data-actor]');if(!a)return;event.preventDefault();state.contextMenu={actorId:a.dataset.actor,x:Math.min(event.clientX,window.innerWidth-200),y:Math.min(event.clientY,window.innerHeight-140)};render();});
  root.addEventListener('pointerover',event=>{const el=event.target.closest('[data-capability]');if(el)showTooltipFor(el,event);});
  root.addEventListener('pointermove',event=>{if(tooltip)moveTooltip(event);});
  root.addEventListener('pointerout',event=>{const el=event.target.closest('[data-capability]');if(!el)return;if(event.relatedTarget&&el.contains(event.relatedTarget))return;clearTooltip();});
  root.addEventListener('focusin',event=>{const el=event.target.closest('[data-capability]');if(!el)return;const r=el.getBoundingClientRect();showTooltipFor(el,{clientX:r.right,clientY:r.top});});
  root.addEventListener('focusout',event=>{if(event.target.closest('[data-capability]'))clearTooltip();});

  root.addEventListener('pointerdown',event=>{const handle=event.target.closest('[data-resize-handle]');if(!handle)return;const pane=handle.closest('.utility-pane');if(!pane)return;resizeState={pane,startX:event.clientX,startW:pane.getBoundingClientRect().width};handle.classList.add('dragging');event.preventDefault();});
  window.addEventListener('pointermove',event=>{if(!resizeState)return;const width=Math.max(288,Math.min(455,resizeState.startW+(resizeState.startX-event.clientX)));resizeState.pane.style.width=`${width}px`;});
  window.addEventListener('pointerup',()=>{root.querySelector('.utility-resize.dragging')?.classList.remove('dragging');resizeState=null;});

  document.querySelector('.review-controls').addEventListener('click',event=>{const jump=event.target.closest('[data-review-jump]');if(jump)applyScenario(jump.dataset.reviewJump);});
  ...
  document.addEventListener('keydown',event=>{if(event.key!=='Escape')return;if(tooltip){clearTooltip();return;}if(state.contextMenu){state.contextMenu=null;render();return;}if(state.action){state.action=null;state.selectedTargets=[];state.notice=null;render();return;}if(state.utility){state.utility=null;render();return;}if(state.fullSheet){state.fullSheet=false;render();}});

  F.scenarios.forEach(s=>{const option=document.createElement('option');option.value=s.id;option.textContent=s.label;scenarioSelect.appendChild(option);});
