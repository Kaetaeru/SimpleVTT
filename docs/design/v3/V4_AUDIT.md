...
- ✔ D295 goliath 거인 혈통 6종 — 풀만 있음. — [data-fix + host-hook]
...
- ✔ D295 하플링 민첩함·은신 본능, 엘프 무아지경 — 표시 없음. — [DM-label]
...
- ✔ D293 produce-flame, flame-blade — 추가 행동 시전·이후 마법 행동 공격이어야. — [data-fix]
...
- ✔ D293 (명중 1d10은 카드로, 2d6 범위는 좌표가 없어 라벨) ice-knife — 피해 없음. 명중 1d10 뒤 범위 2d6 내성. — [new-grammar]
- ✔ D289 command — 효과 없음. — [picker]
- ✔ D269 hideous-laughter — 반복 내성·피해 시 재내성 없음. — [data-fix + host-hook]
- ✔ D294 (확인: 2024 규칙대로 행동불능·속도 0, 피해로 끝남) sleep — 두 번째 실패 무의식 없음. — [new-grammar]
- ✔ D282 heroism — 턴 시작 임시 HP 없음. — [new-grammar]
- ✔ D282 ensnaring-strike, searing-smite — 턴 시작 피해·종료 내성이 문구. — [new-grammar]
- ✔ D290 shining-smite — 대상 공격 유리가 문구(문법 있음). — [data-fix]
- ✔ D283 hellish-rebuke — 반응으로 제시 안 됨. — [host-hook]
- ✔ D290 expeditious-retreat — 판정 표시, 추가 행동 질주 문법 있음. — [data-fix]
- ✔ D294 (좌표 없음 — 사유 있는 라벨) grease — 들어감·턴 끝 내성 없음. — [button]
...
- ✔ D289 dragon-s-breath — 효과 없는 내성, 아군 브레스 없음. — [picker + data-fix]
...
- ✔ D294 (무작위 광선은 사유 있는 라벨) prismatic-spray — 굴림 없음. — [new-grammar]
- ✔ D294 (HP 문턱 표는 사유 있는 라벨) divine-word — HP별 효과 없음. — [new-grammar]
- ✔ D289 eyebite — 선택 없음. — [picker]
- ✔ D294 (턴 끝 반복 내성은 색인으로, d10 행동표는 사유 있는 라벨) confusion — d10 행동·반복 내성 없음. — [new-grammar]
- ✔ D295 polymorph / true-polymorph — 끝난 것처럼 보임. — [DM-label]
- ✔ D269 dispel-evil-and-good — 방향 반대(scope target이어야). — [data-fix]
- ✔ D291 (피해 순간의 재내성은 사유 있는 라벨로 — 앱이 그 창을 아직 열지 않음) dominate-beast/person/monster — 피해 시 즉시 종료(SRD는 재내성). — [data-fix]
- ✔ D291 phantasmal-killer — 판정 불리·반복 피해 문구. — [data-fix]
- ✔ D281 mass-heal — 대상 1명. — [data-fix + picker]
- ✔ D289 conjure-minor-elementals — 항상 화염. — [picker]
