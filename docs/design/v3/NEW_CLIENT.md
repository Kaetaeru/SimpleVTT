...
**M4 착수 — 세션 (2026-09-15, 소유자 결정 "LAN·하마치, 검증 편하게, 시나리오 여러 개")**: 설계는 `CAMPAIGN_RESOURCES.md`, 시나리오는 `SESSION_SCENARIOS.md`. S1 `client/campaign/`(문서 껍데기·kind별 본문·권한 4단계·투영기, 저장소 `documents`), S2 `client/character/ops.ts`(시트 조작 전부를 직렬화 가능한 op 하나의 리듀서 `applyOp`로 — 오프라인 시트와 호스트가 같은 코드를 돌린다; 주사위는 누른 쪽이 굴려 op에 실어 보낸다) + `client/session/`(프로토콜: 명령·번호 붙은 이벤트·스냅샷·초대 코드 / `SessionHost`: 토큰·호스트 비밀 검증, 소유권 검사, DM 개입은 "DM:"으로 기록, 라운드는 호스트가 진행, 재접속 리플레이 버퍼 / `SessionClient`: 미러, 자기 캐릭터 되돌려 쓰기, 거절 알림 / 전송: 메모리 허브, BroadcastChannel(같은 PC의 탭), TCP는 S3), S4 `SheetPlay`(플레이 UI를 op dispatch로 분리해 오프라인 시트와 세션 자리가 같은 컴포넌트) + `SessionScreen`(열기/참가 로비, 파티 카드·DM 빠른 조작·참가자, 가운데 시트(내 것은 조작, 남의 것은 보기, DM은 DM으로 조작), 공용 기록·채팅, 라운드) + `app/session.tsx`. 검사: `session.test.ts` 7건(입장·거절·사칭·DM·라운드·재접속·투영·op), 두 탭 E2E `scripts/capture-client-session.mjs`(SC-1~7, 증거 28–33). 사용자 id는 브라우저에서는 탭마다, exe에서는 PC마다(localStorage) 하나다. **캠페인 기준 세션 (소유자 지적 반영)**: 세션은 캠페인 문서에서 연다. 로비의 "세션 열기"는 캠페인을 고르고(없으면 "새 캠페인"), `#/campaigns`·`#/campaign/<id>` 화면이 캠페인 목록·파티(마지막 HP)·참가자·세션 기록(공용 기록 전체)을 보여 준다. 호스트는 캠페인의 파티와 참가자로 세션을 시드하고(`SessionHost.seed`, 주인이 돌아오기 전에는 연결 끊김으로 표시), 이벤트마다 600ms 디바운스로 `campaignWithSession`이 참가자·파티·세션 기록을 캠페인 문서에 접어 넣으며, 닫을 때 `endedAt`을 찍는다. 다시 데려온 플레이어 사본이 시드를 대체한다(D67). **S3 LAN/하마치**: `client/session/tauriTransport.ts`가 Tauri 셸의 Rust TCP 전송(`session_transport.rs`, 개행 구분 JSON 프레임, 8MB 한도)을 `Transport`로 감싼다 — 호스트는 `0.0.0.0:41230`을 열어 `SessionHost.attach`로 붙이고, 새 Rust 명령 `list_session_addresses`(UDP connect 트릭으로 인터넷·하마치 25.x·사설망 경로의 로컬 주소를 수집, 크레이트 없음)가 준 IP마다 `tcp:<ip>:41230-<token>` 초대를 만든다(첫 번째가 대표, 나머지는 "다른 주소"에). 플레이어는 exe에서 `tcp:` 또는 `IP:포트-토큰`을 붙여넣으면 TCP로 붙고, 브라우저에서는 `tab:`만 받는다. 실패는 로비에 원인(방화벽·호스트 미실행)과 함께 표시된다. 수동 체크리스트는 `SESSION_SCENARIOS.md` §B.
