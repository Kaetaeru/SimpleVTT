...
| `economy.modify` | 행동·추가 행동·반응을 준다 (`bucket`, `amount`). `bonus-action.attack:<무기 범위>`는 그 무기로 추가 행동 공격을 주고, **`amount`가 몇 번인지**다(D342 — 신속한 화살통은 2). `free-attack:<범위>`는 턴을 쓰지 않는다 |
...
- D332: 경제 결제에 `oncePerTurn: true`를 달면 그 사용의 행동은 턴에 한 번만 나간다(주사위를 여러 개 쓰는 사용).
- D331: `spell.attack-roll.ignore-cover`(주문 공격만), 진입점의 `attack.oncePerTurnPerTarget`(대상마다 턴당 한 번), 주문 `primary.healing`의 `hitDice: {count, perSlotAboveBase}`(시전자의 히트 다이스를 써서 회복).
...
| `magic-item-definition` (분류 `magic-item`, D350~) | 붙여넣기 마법 아이템과 같은 형식(`docs/guides/CUSTOM_ITEM_JSON.md`): `type`·`rarity`·`attunement`·`base`·`bonus`(`extraDamage` 포함)·`resistances`·`immunities`·`conditionImmunities`·`abilities`(점수 고정)·`speeds`·`darkvision`·`baseOptions`(지급할 때 기반 선택, D354)·`abilityBonuses`(상한 있는 증가)·`damageType`·`spells[].perLevel/maxLevel`(D356)·`charges`·`spells`·`use`(`healing`·`tempHp`·`effect`·`spell`, D353)·`notes`. 같은 항목에 `common-play` 계약을 달면 아이템이 작동하는 동안 착용자의 특성이 되어 상시 속성·라벨 사용 버튼·반응이 특성과 같은 길로 돈다 (D352). 계약은 사본마다 등록되고(규칙 키 `item.<사본>`) 풀은 `resource:self`(충전)·`resource:self.<uses id>`로 쓴다 — 옛 `resource:<항목 id>`도 첫 사본의 충전으로 읽힌다. 붙여넣기 아이템도 `contract`·`uses`를 똑같이 쓴다 (D358). 형식: `schemas/magic-item.schema.json` |
