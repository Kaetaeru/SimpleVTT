...
| `magic-item-definition` (분류 `magic-item`, D350~) | 붙여넣기 마법 아이템과 같은 형식(`docs/guides/CUSTOM_ITEM_JSON.md`): `type`·`rarity`·`attunement`·`base`·`bonus`(`extraDamage` 포함)·`resistances`·`immunities`·`conditionImmunities`·`abilities`(점수 고정)·`speeds`·`darkvision`·`baseOptions`(지급할 때 기반 선택, D354)·`charges`·`spells`·`use`(`healing`·`tempHp`·`effect`·`spell`, D353)·`notes`. 같은 항목에 `common-play` 계약을 달면(계약 `id` = 항목 id) 아이템이 작동하는 동안 착용자의 특성이 되어 상시 속성·라벨 사용 버튼·반응이 특성과 같은 길로 돈다. 충전 풀은 `resource:<항목 id>`로 쓴다 (D352) |
