...
- `65-totm-opportunity-prompt-dm.png` — 플레이어가 고블린 아이콘의 "🏃 벗어남"을 누른 뒤 DM 화면: 보드 오른쪽 아래 승인 도크("⚔ 시미터" / "안 함")와 채팅의 같은 프롬프트 카드, 이니셔티브·턴 토스트, 강제 치명타 카드(vs AC 18 (엄폐 +2))
...
- `67-turn-panel-player.png` — 플레이어 화면, 자기 캐릭터의 턴: 한 줄 턴 패널(⚔ 무기·붙잡기·밀치기·벗어나기 | 행동·판정·특성·아이템·추가 행동 ▾ | 턴 마침, 행동 경제 칩), 회피·영향 카드, 운동 판정 카드, 행동 폭증·재기의 바람 사용 토스트, 아이콘의 🛡 마크
