...
- 증거: `npm run test:table` 45건, `tsc`, 경계 검사 둘, `vite build` 모두 green (Linux 오프라인). 화면(T2-04)·Windows 실기기·전환(T2-07)은 여전히 하지 않았다.
- V1 완료 기준으로 남은 커널 항목: 바드 영감 사용, 무기 숙련, 사냥꾼의 표식·신성한 강타, 불굴, 야생 변신, 몬스터 재충전·전설 행동·다중 공격·명중 부가 효과, 공포·투명 공격 수정, write-back 나머지 내구 항목. 그다음이 화면과 Windows 검증이다.
