...
| V1-31~32 Connected play | PARTIAL | 핵심 connected suites + remote-owner Rage/Wild Shape/Cunning Action Dash/Disengage/Hide/Uncanny Dodge/Berserker Intimidating Presence/Open Hand Wholeness of Body/Open Hand Fleet Step/Devotion Holy Nimbus/Open Hand Quivering Palm forward·exactly-once·Undo focused evidence green; 나머지 R1 remote-owner/reconnect matrix 및 two-instance proof 대기 |
...
- [x] Session chrome에 Campaign 시간/시간대를 상시 표시하고 DM hover/focus 직접 시·분 수정 제공.
...
R2 focused evidence now includes remote-owner Rage (`dec4f22`), Wild Shape (`a65cbd2` proof), Cunning Action Dash (`ea96509` proof), Cunning Action Disengage (`134e6a8` proof), Cunning Action Hide (exact green tree `7f8e945`), Uncanny Dodge (exact green test head `a1edf6b`), Berserker Intimidating Presence (exact green test head `3d3c986`), Open Hand Wholeness of Body (exact green test/content head `d03adbe`), Open Hand Fleet Step (focused proof `f2c764b`, gate wiring `52d2f6f`, exact actor-scoped fix head `df37d8a`), Devotion Holy Nimbus (focused proof `9e426db`, gate wiring `2e689f9`, canonical Devotion catalog `b326337`, exact green tree head `5ff7d00`), and Open Hand Quivering Palm (freeform proof `09d0cea`, feature-local initiative persistence final head `37d0028`). Quivering Palm exact-head UI run `32979538192` / frontend job `98212492938` and Phase 12 run `32979538159` / connected-protocol job `98212492528` are green through initiative Action economy, marker persistence, owner exactly-once, reconnect and compensating Undo. These slices prove Host-unknown projection/authority and ordered event convergence without transferring permanent Host Character ownership. They are focused evidence only; V1-31 remains PARTIAL until the remaining R1 remote-owner/reconnect matrix is covered.
...
R2 focused slices through Open Hand Quivering Palm reuse the same duplicate-safe event ledger and compensating Undo foundations. Rage confirms owning-Client durable forward/inverse write-back with Host permanent-library isolation; Wild Shape and Cunning Action slices extend Host-unknown authoritative execution through class feature state/economy/effects; Cunning Hide proves d20 + Bonus Action + tagged Hidden effect convergence. Uncanny Dodge additionally proves private owner interrupt acceptance, authoritative Reaction + HP ordered events, atomic floor-half damage, owner HP durable apply exactly once, duplicate Host event and duplicate owner response no-op, stale reconnect preservation, compensating Undo, inverse owner write-back and duplicate Undo safety at exact green test head `a1edf6b`. Berserker Intimidating Presence adds canonical subclass reconstruction, target/resource/Bonus Action/Frightened event convergence, owner durable apply exactly once, duplicate request/event safety, reconnect preservation and compensating inverse persistence at exact green head `3d3c986`. Open Hand Wholeness of Body adds canonical subclass reconstruction, HP/resource/Bonus Action ordered events, owner durable apply exactly once, duplicate request/event safety, reconnect/rebind preservation and compensating inverse persistence at exact green head `d03adbe`. Open Hand Fleet Step adds authoritative prior non-Step Bonus Action trigger preservation, Focus/resource and movement/effect ordered events, Host permanent-library isolation, owner durable apply exactly once, duplicate request/event no-op, reconnect/rebind, compensating Undo and inverse owner persistence at exact green head `df37d8a`. Devotion Holy Nimbus adds canonical Devotion reconstruction, resource/Bonus Action/self-effect ordered events, Host permanent-library isolation, owner durable apply exactly once, duplicate request/event safety, reconnect/fresh projection, compensating Undo and inverse owner convergence at exact green tree head `5ff7d00`. Open Hand Quivering Palm adds Focus/resource + marker/effect + initiative Action economy + save/damage ordered convergence, Host permanent-library isolation, owner exactly-once apply, duplicate safety, reconnect/rebind, feature-local freeform→initiative marker persistence and compensating Undo/inverse owner convergence at exact green head `37d0028`. The remaining R1 matrix and actual two-instance/restart proof are still pending, so V1-32 remains PARTIAL.
...
```text
R2 Devotion Smite of Protection remote-owner gap
```

1. R1 source/execution action matrix는 canonical handoff에서 DONE이다. 이 판정은 V1-21 release DONE이나 Windows/human acceptance가 아니다.
2. R2에서 remote-owner Rage, Wild Shape, Cunning Action Dash/Disengage/Hide, Uncanny Dodge, Berserker Intimidating Presence, Open Hand Wholeness of Body, Open Hand Fleet Step, Devotion Holy Nimbus, Open Hand Quivering Palm은 focused evidence와 exact production gates가 green이다. Quivering Palm exact green head는 `37d002862a9ac253b8b7e6b0b138369c588be17d`; UI `32979538192` / frontend `98212492938`, Phase 12 `32979538159` / connected-protocol `98212492528`가 success다. 이 slice들을 반복하지 않는다.
3. 다음은 Devotion Smite of Protection remote-owner gap이다. R1 exact checkpoint `ec89fa251d969a250c20e11f0abe6d7a4f13d58e`의 기존 committed Divine Smite 후 자동 protection append, aura marker/expiry, Activity, ResolutionEvent/Undo를 재사용한다. 별도 Smite-of-Protection 버튼을 만들지 않는다.
4. eligible Host-unknown Devotion Paladin의 committed Divine Smite를 Host authoritative resolve하고 spell/resource/economy + protection-effect ordered events로 수렴하는지 focused deterministic proof로 검증한다. Host permanent Character library 불변과 owning Client exactly-once apply/persist를 확인한다.
5. duplicate request/event no-op, reconnect/fresh projection, marker/expiry 수렴, compensating Undo의 Host projection + owner inverse convergence를 확인한다.
6. 실제 red가 있으면 첫 Smite-of-Protection 관련 원인 하나만 최소 수정하고 exact SHA UI frontend + Phase12 connected production gate를 확인한다. 새 protocol/schema/rules engine은 추가하지 않는다.
