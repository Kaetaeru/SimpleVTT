...
test("portable reaction lowering is invariant to definition and interaction identities",()=>{
  ...
  assert.deepEqual(first.interceptors.map(({id,interaction,...rest})=>({...rest,interaction:{...interaction,id:"ignored"}})),
    renamed.interceptors.map(({id,interaction,...rest})=>({...rest,interaction:{...interaction,id:"ignored"}})));
});

test("unconnected interceptor families stay data-only instead of creating a side engine",()=>{
  ...
  assert.equal(lowerCommonPlayReactionDefinition(definition),undefined);
});
