...
const FIXTURE=JSON.parse(readFileSync(
  new URL("../fixtures/play-contract/action-economy-resource.json",import.meta.url),
  "utf8",
...
test("validated external Common Play action economy fixture spends resources atomically and grants a profile-defined restricted action", () => {
...
test("generic action economy composition matches the Action Surge golden state changes without knowing Fighter content identity", () => {
  ...
  definition.entryPoints[0].operations[0]={
    kind:"resource.change",
    resource:FIGHTER_ACTION_SURGE_RESOURCE_ID,
    amount:{ value:-1 },
  };
  definition.entryPoints[0].operations[1]={
    kind:"resource.change",
    resource:FIGHTER_ACTION_SURGE_TURN_RESOURCE_ID,
    amount:{ value:-1 },
  };
