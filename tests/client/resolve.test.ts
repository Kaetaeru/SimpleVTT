...
const sword: AttackSpec = { name: "장검", source: "weapon", attackBonus: 5, mode: "melee", rangeFeet: 5, damage: [{ formula: "1d8+3", type: "참격" }] };
...
test("rules table: advantage from conditions, AC, natural 20/1, crit doubling, resistances in either language, temp HP, concentration, downed", () => {
  ...
  const bow: AttackSpec = { ...sword, name: "단궁", mode: "ranged", rangeFeet: 80, longRangeFeet: 320, damage: [{ formula: "1d6+2", type: "piercing" }] };
  ...
  // Range: beyond normal range = disadvantage; beyond long range = miss.
  result = resolveAttack(me, combatant({ distanceFeet: 100 }), bow, { dice: scripted(19, 2) });
  assert.deepEqual([result.advantage, result.reasons], ["disadvantage", ["긴 사거리"]]);
  assert.equal(resolveAttack(me, combatant({ distanceFeet: 400 }), bow, { dice: scripted(19, 19) }).outcome, "miss");
...
test("sheet specs: weapon ranges, riders (암습, 신성한 강타 with a slot), PC combatant with concentration, NPC from token bar", () => {
  ...
  assert.deepEqual(weaponRange(dagger), { mode: "melee", rangeFeet: 5 });
  assert.deepEqual(weaponRange(rogueDerived.attacks.find((attack) => attack.name === "단궁")!), { mode: "ranged", rangeFeet: 80, longRangeFeet: 320 });
  ...
  const token = { ...tokenForNpc(goblin, { x: 0, y: 0 }), bars: [{ value: 4, max: 10, visible: true, editable: false }, { visible: true, editable: false }, { visible: true, editable: false }] as never };
...
test("host: a player's ⚔ hits an NPC token (bar drops, 사망 marker), the goblin hits back into the sheet, the palette edits and undoes, D90 waits", async () => {
  ...
  const goblinToken = tokenForNpc(goblin, { x: 2, y: 1 });
  ...
  assert.equal(card.action!.distanceFeet, 5);
