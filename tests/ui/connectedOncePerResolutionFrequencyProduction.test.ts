...
function packageJson(prefix:string) {
  ...
  return {
    ...
    json:JSON.stringify({
      ...
      source:{document:"Unknown once-per-resolution frequency proof",version:"1",license:"CC0",srdDerived:false},
      dependencies:[],conflicts:[],capabilities:[],
      content:[
        {id:`${prefix}.feat`,category:"feat",presentation:{defaultLocale:"en",originalName:"Unknown Resolution Rider",locales:{en:{name:"Unknown Resolution Rider"}}},mechanics:[{kind:"common-play",config:{
          ...
          artifactTemplates:[{
            ...
            rules:[{id:"once-resolution",event:"damage.dealt",frequency:"once-per-resolution",operations:[
              {kind:"damage.apply",amount:{value:3},damageType:"force",target:"event.actor"},
            ]}],
            lifetime:{kind:"until-duration",onEnd:"destroy"},
          }],
        }}]},
        {id:`${prefix}.action`,category:"option",presentation:{defaultLocale:"en",originalName:"Unknown Double Damage",locales:{en:{name:"Unknown Double Damage"}}},mechanics:[{kind:"common-play",config:{
...
test("once-per-resolution automatic frequency fires once inside one multi-damage resolution and re-arms for the next resolution",async()=>{
  const expected={firstSourceDelta:3,firstTargetDelta:4,secondSourceDelta:6,secondTargetDelta:8,markerCount:1};
  assert.deepEqual(await runIdentity("unknown-once-resolution-a"),expected);
  assert.deepEqual(await runIdentity("fully-renamed-once-resolution-b"),expected);
});

test("once-per-resolution marker converges, reconnects, deduplicates, and restores through event-native Undo",async()=>{
  const prefix="unknown-connected-once-resolution",sessionId="session.once-per-resolution";
  ...
  assert.equal(totalHealth(first,"combatant.goblin-a"),totalHealth(before,"combatant.goblin-a")-4);
