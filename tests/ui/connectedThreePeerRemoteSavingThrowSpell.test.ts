...
const THUNDERWAVE="dnd.srd521.spell.thunderwave";
...
async function createSpellcasterFixture(host:MockAdapter,characterId:string,name:string,revision:number):Promise<PlayerFixture>{
  ...
  contentEntry(snapshot.catalog,THUNDERWAVE);
...
async function assertSamePublicSavePresentation(
  ...
  assert.ok(live.length>=1,"remote saving-throw spell must publish live presentation before terminal commit");
  assert.equal(batches.length,1,"remote saving-throw spell must commit exactly one terminal event batch");
  ...
  assert.ok(live.every((message)=>message.presentation.action?.id===actionId));
  assert.ok(live.every((message)=>message.presentation.action?.saveDc===saveDc),"policy-public Character spell DC must remain identical for P1 and P2");
  assert.ok(live.every((message)=>message.presentation.action?.saveAbility===saveAbility),"policy-public save ability must remain identical for P1 and P2");

  const resultPresentation=live.find((message)=>message.presentation.resolution.saveResults.length===targetIds.length);
  assert.ok(resultPresentation,"live public presentation must include one Host save result for every selected target");
  assert.deepEqual(resultPresentation.presentation.resolution.saveResults.map((result)=>result.targetId),targetIds,"each public save result must stay associated with its Host-selected target");
  assert.ok(resultPresentation.presentation.resolution.saveResults.every((result)=>result.dc===saveDc),"each public save result must use the same authoritative Character spell DC");
  assert.deepEqual(resultPresentation.presentation.targets.map((target)=>target.id),targetIds,"public target labels must retain Host target order");

  const terminal=batches[0].events.find((event)=>event.payload.kind==="resolution");
  assert.ok(terminal&&terminal.actorId===actorId,"terminal saving-throw spell event must belong to P1");
  if(!terminal||terminal.payload.kind!=="resolution")throw new Error("missing terminal saving-throw spell resolution event");
  assert.equal(terminal.payload.presentation.action?.saveDc,saveDc);
  assert.equal(terminal.payload.presentation.action?.saveAbility,saveAbility);
  assert.deepEqual(terminal.payload.presentation.resolution.saveResults.map((result)=>result.targetId),targetIds,"terminal save results must retain exact Host target association");
  assert.ok(terminal.payload.presentation.resolution.saveResults.every((result)=>result.dc===saveDc));
  ...
  while(true){
    const [acting,observing]=await Promise.all([actingClient.getSnapshot(),observingClient.getSnapshot()]);
    assert.ok(acting.resolution&&observing.resolution,"both remote consumers must retain the shared public save presentation");
  ...
  assert.deepEqual(observingStages,actingStages,"P1 and P2 must consume the same ordered public saving-throw spell presentation");
}

test("MP-C13 core · remote P1 Thunderwave keeps every Host save result bound to the correct target and exposes the same public DC to P2",async()=>{
  ...
  try{
    ...
    const p1=await createSpellcasterFixture(host,"char.mp-c.save-p1","MP-C Save P1",1109);
    const p2=await createSpellcasterFixture(host,"char.mp-c.save-p2","MP-C Save P2",1201);
    ...
    assert.equal(state.peerParticipants.size,2,"Host must retain P1 and P2 for saving-throw spell spectator fan-out");

    const before=await host.getSnapshot();
    const thunderwave=before.scene.actionsByActor[p1.characterId]?.find((action)=>action.id==="action.spell.thunderwave");
    assert.ok(thunderwave,"Host projection must expose P1 canonical Thunderwave");
    assert.equal(thunderwave.resolutionKind,"saving-throw");
    assert.equal(thunderwave.spellCast?.spellId,THUNDERWAVE);
    assert.equal(thunderwave.spellCast?.baseLevel,1);
    assert.ok(thunderwave.saveDc,"projected Thunderwave must expose the Character-derived save DC");
    assert.ok(thunderwave.saveAbility,"projected Thunderwave must expose its save ability");
    assert.equal(thunderwave.saveDc,13,"level-1 CHA 16 Sorcerer with proficiency +2 must expose public spell save DC 13");

    const targetIds=thunderwave.eligibleTargetIds
      .filter((id)=>before.scene.entities.find((entity)=>entity.id===id)?.side==="enemy")
      .slice(0,2);
    assert.equal(targetIds.length,2,"MP-C13 needs two Host Scene targets to prove per-target save association");
    ...
    transport.emitFrom("peer.mp-c.save-p1",{
      ...
      request:{
        ...
        requestId:"mp-c13.remote-thunderwave",
    ...
    assert.equal(completed.resolution?.actionId,thunderwave.id);
    assert.deepEqual(completed.resolution?.targetIds,targetIds,"Host resolution must retain exact selected target order");
    assert.deepEqual(completed.resolution?.saveResults.map((result)=>result.targetId),targetIds,"Host must bind one save result to each selected target in order");
    assert.ok(completed.resolution?.saveResults.every((result)=>result.dc===thunderwave.saveDc),"Host save results must all use the authoritative Character spell DC");
    assert.equal(state.ledger.cursor,actionCursor+1,"remote saving-throw spell must commit exactly one Host ledger event");

    await assertSamePublicSavePresentation(
