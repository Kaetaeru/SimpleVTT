...
import { ensureCoreClassResources } from "../../src/app/classFeatureSpellRuntimeAdapter";
import { FIEND_DARK_ONES_OWN_LUCK_RESOURCE_ID } from "../../src/domain/warlockFiend";
...
function remoteFiend(catalog:CatalogEntry[]):CharacterSheet {
  const warlock=entry(catalog,WARLOCK_ID);
  const fiend=entry(catalog,WARLOCK_FIEND_SUBCLASS_ID);
  const human=entry(catalog,"dnd.srd521.species.human");
  const soldier=entry(catalog,"dnd.srd521.background.soldier");
  const sheet={
    id:CHARACTER_ID,name:"Remote Fiend Warlock",className:warlock.nameKo||warlock.nameEn,subclassName:fiend.nameKo||fiend.nameEn,level:6,
    species:human.nameKo||human.nameEn,background:soldier.nameKo||soldier.nameEn,hp:45,maxHp:45,tempHp:0,ac:13,speed:30,proficiencyBonus:3,saveState:"saved",
    abilities:{str:10,dex:14,con:14,int:12,wis:12,cha:18},saves:[],skills:[],features:["어둠의 존재의 행운"],equipment:[],items:[],attacks:[],resources:[],
    rulesProfileId:"dnd.srd-5.2.1",rulesProfileVersion:"0.1-draft",sourceRevision:2,runtimeRevision:3,
    classLevels:[{classId:WARLOCK_ID,className:warlock.nameKo||warlock.nameEn,level:6,subclassName:fiend.nameKo||fiend.nameEn}],subclassIds:{[WARLOCK_ID]:WARLOCK_FIEND_SUBCLASS_ID},
  } as CharacterSheet;
  return ensureCoreClassResources(sheet);
}
...
function luckCurrent(sheet:CharacterSheet) {
  return sheet.resources.find((resource)=>resource.id===FIEND_DARK_ONES_OWN_LUCK_RESOURCE_ID)?.current;
}

test("host-unknown Fiend Dark One's Own Luck accepts owner interrupt, spends one use, reconnects, and Undo compensates",async()=>{
  const host=new MockAdapter();await host.setReferenceRole("dm");
  const before=await host.getSnapshot();const catalog=structuredClone(before.catalog);const remote=remoteFiend(catalog);const remoteManifest=manifest(remote);const projection=buildCharacterSessionProjectionV1(remote,catalog);
  ...
  try {
    ...
    const request:ConnectedActionRequest={sessionId:state.sessionId,requestId:"request.r2.remote-dark-ones-own-luck",actorId:remote.id,actionId:check!.id,targetIds:[],knownEventCursor:0,character:remoteManifest.character,capabilities:[...CONNECTED_CAPABILITIES]};
    ...
    assert.equal(snapshot.resolution?.stage,"complete");assert.deepEqual(snapshot.resolution?.authoritativeDice,[4,10]);assert.equal(snapshot.resolution?.rollTotal,14);assert.equal(snapshot.resolution?.checkOutcome,"성공");
    ...
    if(hostEvent!.payload.kind!=="resolution")throw new Error("expected Host Dark One resolution event");
    const changes=hostEvent!.payload.resolutionEvents.flatMap((event)=>event.stateChanges);assert.ok(changes.some((change)=>change.kind==="resource"&&change.targetId===remote.id&&change.resourceId===FIEND_DARK_ONES_OWN_LUCK_RESOURCE_ID&&change.before===beforeUses&&change.after===beforeUses!-1));
    ...
    let clientAfter=await client.getSnapshot();assert.equal(luckCurrent(clientAfter.activeCharacter),beforeUses!-1);
    const persistenceAfter=getCharacterLibraryPersistenceStateForTests(client)?.storageRevision??0;assert.ok(persistenceAfter>persistenceBefore,"owning Client must persist the Host-confirmed Dark One resource spend before cursor advancement");
    ...
    snapshot=await host.getSnapshot();assert.equal(luckCurrent(projectedCharacterById(host,remote.id)!.sheet),beforeUses!-1,"stale reconnect projection must not restore the spent use");assert.equal(projectedCharacterById(host,remote.id)?.peerId,RECONNECT_PEER);assert.deepEqual(snapshot.characters,before.characters);

    const persistenceBeforeUndo=getCharacterLibraryPersistenceStateForTests(client)?.storageRevision??0;
    await host.undoLastResolution();snapshot=await host.getSnapshot();assert.equal(luckCurrent(projectedCharacterById(host,remote.id)!.sheet),beforeUses);assert.deepEqual(snapshot.characters,before.characters);
    ...
    clientAfter=await client.getSnapshot();assert.equal(luckCurrent(clientAfter.activeCharacter),beforeUses);
    const persistenceAfterUndo=getCharacterLibraryPersistenceStateForTests(client)?.storageRevision??0;assert.ok(persistenceAfterUndo>persistenceBeforeUndo,"owning Client must persist the compensating Dark One resource restore before cursor advancement");
