...
async function assertTwoRemoteSaveConsumers(messages:ConnectedWireMessage[],actorId:string,actionId:string,targetId:string,d20:number){
  ...
  assert.ok(live.length>=1,"remote saving throw must publish a live presentation before terminal commit");
  assert.equal(batches.length,1,"remote saving throw must commit exactly one terminal event batch");
  ...
  assert.ok(
    live.some((message)=>
      ...
      &&message.presentation.resolution.stage==="save-animation"
      &&message.presentation.resolution.authoritativeDice.includes(d20)
    ),
    "live saving-throw presentation must expose the Host-authoritative save die",
  );

  const terminal=batches[0].events.find((event)=>event.payload.kind==="resolution");
  assert.ok(terminal&&terminal.actorId===actorId,"terminal saving-throw event must belong to P1");
  if(!terminal||terminal.payload.kind!=="resolution")throw new Error("missing terminal saving-throw resolution event");
  ...
  while(true){
    const [acting,observing]=await Promise.all([actingClient.getSnapshot(),observingClient.getSnapshot()]);
    assert.ok(acting.resolution&&observing.resolution,"both remote consumers must retain the shared live saving-throw presentation");
  ...
  assert.deepEqual(observingStages,actingStages,"P1 and P2 must consume the same ordered public saving-throw presentation");
}
...
test("MP-C09 core · remote P1 saving throw action resolves once on Host and fans out the same save presentation to P2",async()=>{
  ...
  try{
    ...
    const p1=await createPlayerFixture(host,"char.mp-c.save-p1","MP-C Save P1",707);
    const p2=await createPlayerFixture(host,"char.mp-c.save-p2","MP-C Save P2",808);
    ...
    await assertTwoRemoteSaveConsumers(transport.broadcastsAfter(broadcastStart),p1.characterId,shove.id,target.id,authoritativeD20);
