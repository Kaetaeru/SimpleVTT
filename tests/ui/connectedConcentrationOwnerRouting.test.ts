...
test("concentration d20 prompt is owner-only and the Host validates the responding peer",async()=>{
  ...
  try{
    ...
    for(const wire of broadcasts.map(JSON.parse).filter((entry)=>entry.type==="resolution-presentation")){assert.equal(wire.presentation.resolution.concentrationSave,undefined,`no public presentation carries the concentration save before the owner answers (stage ${wire.presentation.resolution.stage})`);assert.doesNotMatch(JSON.stringify(wire),/modifierSource/);}
