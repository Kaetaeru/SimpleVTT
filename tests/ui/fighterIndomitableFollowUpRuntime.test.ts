...
test("failed Fighter save offers Indomitable, uses the reroll, and Undo restores HP and resource",async()=>{
  ...
  await adapter.setQueuedD20(1);await adapter.resolveAction(SAVE_ACTION,[fighter.id]);snapshot=await adapter.advanceResolution();assert.equal(snapshot.resolution?.interrupt?.id,"follow-up.fighter.indomitable",JSON.stringify({resolution:snapshot.resolution,resources:snapshot.activeCharacter.resources}));
  await adapter.setQueuedD20(10);snapshot=await adapter.respondToInterrupt(true);assert.equal(snapshot.resolution?.saveResults[0]?.outcome,"성공");assert.equal(snapshot.activeCharacter.resources.find((entry)=>entry.id===FIGHTER_INDOMITABLE_RESOURCE_ID)?.current,beforeUses-1);
