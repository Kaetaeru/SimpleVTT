...
test("failed Fighter save offers Indomitable, uses the reroll, and Undo restores HP and resource",async()=>{
  ...
  await adapter.setQueuedD20(1);await adapter.resolveAction(SAVE_ACTION,[fighter.id]);snapshot=await adapter.advanceResolution();assert.equal(snapshot.resolution?.interrupt?.id,"follow-up.d20-modification",JSON.stringify({resolution:snapshot.resolution,resources:snapshot.activeCharacter.resources}));
