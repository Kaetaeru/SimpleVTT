...
function supportedCommonPlayMechanic() {
  return {
    kind:"common-play",
    config:{
...
test("supported Common Play mechanics survive RuleModule install and rehydrate as data", async () => {
  ...
  setInstalledContentStoreForTests(writer,store);
  const raw=JSON.parse(packagePayload()) as {content:Array<Record<string,unknown>>};
  const mechanic=supportedCommonPlayMechanic();
  ...
  const persisted=getInstalledContentPersistenceStateForTests(writer)?.document?.entries.find((entry)=>entry.contentId==="option.atomic-parent");
  assert.deepEqual((persisted as unknown as {mechanics?:unknown})?.mechanics,[mechanic]);
  ...
  assert.deepEqual((restored as unknown as {mechanics?:unknown})?.mechanics,[mechanic]);
});
