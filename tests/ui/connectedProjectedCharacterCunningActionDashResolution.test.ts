...
import type { AppSnapshot, CatalogEntry, CharacterSheet, CharacterSummary, SceneVm } from "../../src/app/contracts";
...
function remoteRogue(catalog:CatalogEntry[]):CharacterSheet {
  ...
  return {
    ...
    saves:[],skills:["은신","손재주"],features:["교활한 행동"],equipment:[],items:[],attacks:[],resources:[],
...
function economy(snapshot:AppSnapshot,actorId:string) {
  const value=snapshot.scene.economyByActor[actorId];
  assert.ok(value,`missing economy for ${actorId}`);
  return value!;
}

test("host-unknown Rogue Cunning Action Dash commits session economy through Host authority and compensating Undo exactly once",async()=>{
  ...
  const mountedBefore=await host.getSnapshot();
  const beforeEconomy=structuredClone(economy(mountedBefore,remote.id));
  ...
  try {
    ...
    assert.equal(state.pendingRemoteAction?.request.requestId,request.requestId,"Host must accept the projected Cunning Action before advancing its staged resolution");
    await host.advanceResolution();

    const completed=await host.getSnapshot();
    assert.equal(completed.activeCharacter.id,before.activeCharacter.id,"Host local Character context must restore after remote Cunning Action commit");
    ...
    const hostEconomy=economy(completed,remote.id);
    assert.equal(hostEconomy.action,true,"Cunning Action must not spend the normal Action");
    assert.equal(hostEconomy.bonusAction,false,"Cunning Action must spend the Bonus Action");
    assert.equal(hostEconomy.movementMax,beforeEconomy.movementMax+remote.speed);
    assert.equal(hostEconomy.movement,beforeEconomy.movement+remote.speed);
    ...
    assert.equal(batches.length,1,"remote Cunning Action Dash must commit exactly one ordered event batch");
    ...
    assert.ok(changes.some((change)=>change.kind==="economy"&&change.targetId===remote.id&&change.field==="movementMaximum"&&change.before===beforeEconomy.movementMax&&change.after===beforeEconomy.movementMax+remote.speed));
    ...
    let clientEconomy=economy(await client.getSnapshot(),remote.id);
    assert.equal(clientEconomy.bonusAction,false);
    assert.equal(clientEconomy.movementMax,beforeEconomy.movementMax+remote.speed);

    const duplicate=await applyConnectedClientEvents(client,[hostEvent!]);
    assert.equal(duplicate.status,"duplicate");
    clientEconomy=economy(await client.getSnapshot(),remote.id);
    assert.equal(clientEconomy.movementMax,beforeEconomy.movementMax+remote.speed,"duplicate Host event must not apply Dash movement twice");
    ...
    const hostUndoEconomy=economy(hostAfterUndo,remote.id);
    assert.equal(hostUndoEconomy.bonusAction,beforeEconomy.bonusAction);
    assert.equal(hostUndoEconomy.movementMax,beforeEconomy.movementMax);
    assert.equal(hostUndoEconomy.movement,beforeEconomy.movement);
    assert.equal(hostAfterUndo.activeCharacter.id,before.activeCharacter.id,"Host local Character context must remain restored after remote Cunning Action Undo");
    ...
    assert.equal(batchesAfterUndo.length,2,"remote Cunning Action Undo must publish one compensating ordered event batch");
    ...
    clientEconomy=economy(await client.getSnapshot(),remote.id);
    assert.equal(clientEconomy.bonusAction,beforeEconomy.bonusAction);
    assert.equal(clientEconomy.movementMax,beforeEconomy.movementMax);
    assert.equal(clientEconomy.movement,beforeEconomy.movement);

    const duplicateUndo=await applyConnectedClientEvents(client,[undoEvent!]);
    assert.equal(duplicateUndo.status,"duplicate");
    clientEconomy=economy(await client.getSnapshot(),remote.id);
    assert.equal(clientEconomy.movementMax,beforeEconomy.movementMax,"duplicate Undo must not compensate twice");
  } finally {
