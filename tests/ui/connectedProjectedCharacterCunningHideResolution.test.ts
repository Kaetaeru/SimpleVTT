...
import { CUNNING_HIDE_ACTION_ID, ROGUE_CLASS_ID } from "../../src/app/rogueCoreRuntimeAdapter";

const PEER="peer.r2.remote-cunning-hide";
const CHARACTER_ID="char.r2.remote-rogue-hide";
...
function remoteRogue(catalog:CatalogEntry[]):CharacterSheet {
  ...
  return {
    id:CHARACTER_ID,name:"Remote Hide Rogue",className:rogue.nameKo||rogue.nameEn,level:2,species:human.nameKo||human.nameEn,background:soldier.nameKo||soldier.nameEn,
    ...
    saves:[],skills:["은신 +5"],features:["교활한 행동"],equipment:[],items:[],attacks:[],resources:[],rulesProfileId:"dnd.srd-5.2.1",rulesProfileVersion:"0.1-draft",sourceRevision:2,runtimeRevision:3,
...
const hidden=(scene:SceneVm,id:string)=>scene.entities.find((entity)=>entity.id===id)?.status.some((status)=>status.endsWith("숨음"))??false;

test("host-unknown Rogue Cunning Hide converges d20/effect/economy exactly once and Undo compensates",async()=>{
  ...
  const state=connectedStateFor(host);state.mode="host";state.sessionId="session.r2.remote-cunning-hide";state.ledger=new HostSessionLedger(state.sessionId,connectedManifest(host));state.peerManifests.set(PEER,structuredClone(remoteManifest));
  ...
  try {
    ...
    const request:ConnectedActionRequest={sessionId:state.sessionId,requestId:"request.r2.remote-cunning-hide",actorId:remote.id,actionId:CUNNING_HIDE_ACTION_ID,targetIds:[],knownEventCursor:0,character:remoteManifest.character,capabilities:[...CONNECTED_CAPABILITIES]};
    ...
    let clientAfter=await client.getSnapshot();assert.equal(hidden(clientAfter.scene,remote.id),true);assert.equal(clientAfter.scene.economyByActor[remote.id]?.bonusAction,false);assert.equal(clientAfter.scene.economyByActor[remote.id]?.action,true);assert.equal(getCharacterLibraryPersistenceStateForTests(client)?.storageRevision??0,persistenceBefore);
    ...
    await host.undoLastResolution();const hostUndo=await host.getSnapshot();assert.equal(hidden(hostUndo.scene,remote.id),false);assert.equal(hostUndo.scene.economyByActor[remote.id]?.bonusAction,economyBefore!.bonusAction);
    ...
    clientAfter=await client.getSnapshot();assert.equal(hidden(clientAfter.scene,remote.id),false);assert.equal(clientAfter.scene.economyByActor[remote.id]?.bonusAction,economyBefore!.bonusAction);assert.equal(getCharacterLibraryPersistenceStateForTests(client)?.storageRevision??0,persistenceBefore);
