...
test("Campaign runtime projects live Session calendar and ration state without duplicating the aggregate",async()=>{
  ...
  await adapter.upsertCampaignRosterMember("campaign.session-systems",{rosterMemberId:"member.one",label:initial.activeCharacter.name,kind:"player-character-ref",characterRef:{characterId:initial.activeCharacter.id},active:true,countsForRations:true,rationUnitsPerDay:2});
  await adapter.upsertCampaignRosterMember("campaign.session-systems",{rosterMemberId:"member.absent",label:"Absent Hero",kind:"host-preset",active:true,countsForRations:true,rationUnitsPerDay:5});
  ...
  assert.equal(snapshot.campaignSessionSystems?.roster.find((member)=>member.rosterMemberId==="member.one")?.label,initial.activeCharacter.name);
  assert.equal(snapshot.campaignSessionSystems?.roster[0].countsForRations,true);
  assert.equal(snapshot.campaignSessionSystems?.roster.find((member)=>member.rosterMemberId==="member.one")?.presentInSession,true);
  assert.equal(snapshot.campaignSessionSystems?.roster.find((member)=>member.rosterMemberId==="member.absent")?.presentInSession,false);
  ...
  await adapter.serveCampaignMeals("campaign.session-systems",{rosterMemberIds:["member.one"],mealUnits:1,source:"camp"});
  snapshot=await adapter.getSnapshot();
  assert.deepEqual(snapshot.campaignSessionSystems?.rations.mealsByRosterMember,{"member.one":1});
  assert.equal(snapshot.campaignSessionSystems?.rations.mealsShortage,1);
