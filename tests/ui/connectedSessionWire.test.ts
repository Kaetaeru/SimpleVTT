...
test("connected wire round-trips handshake, action request, catch-up, and event batch envelopes", () => {
  ...
  roundTrip({ type:"event-batch",sessionId:"session.test",afterCursor:0,events:[event] });
  roundTrip({ type:"error",code:"stale-cursor",message:"client is behind",hostCursor:3 });
});

test("connected wire rejects malformed JSON, unknown message types, and invalid cursors", () => {
