...
export function CampaignDmLibraryOrganizationPanel({campaign}:{campaign:CampaignRecordV1}){
  ...
  const renameFolder=(folderId:string,currentLabel:string)=>perform(async()=>{
    const label=(folderEdits[folderId]??currentLabel).trim();if(!label)throw new Error("폴더 이름을 입력하세요.");
    await mockAdapter.upsertCampaignDmLibraryFolder(campaign.campaignId,{folderId,label});
    setFolderEdits((current)=>{const next={...current};delete next[folderId];return next;});
  });
  ...
  const savePreset=()=>perform(async()=>{
    ...
    const previous=editingPresetId?campaign.dmLibrary.entries.find((entry)=>entry.entryId===editingPresetId&&entry.kind==="pc-preset"):undefined;
    const entryId=previous?.entryId??localId("dm-pc-preset");
  ...
  const deletePreset=(entryId:string)=>perform(async()=>{await api.removeCampaignDmLibraryEntry(campaign.campaignId,entryId);if(editingPresetId===entryId)resetPreset();});
  ...
  return <section className="campaign-capability-note" aria-label="DM 라이브러리 PC preset과 폴더">
    ...
    {folders.length>0&&<div className="campaign-option-list">{folders.map((folder)=><div key={folder.folderId}><span><strong>{folder.label}</strong><small>{campaign.dmLibrary.entries.filter((entry)=>entry.folderId===folder.folderId).length}개 항목</small></span><input aria-label={`${folder.label} 이름`} value={folderEdits[folder.folderId]??folder.label} disabled={busy} onChange={(event)=>setFolderEdits((current)=>({...current,[folder.folderId]:event.target.value}))}/><button type="button" disabled={busy} onClick={()=>void renameFolder(folder.folderId,folder.label)}>이름 저장</button><button type="button" disabled={busy} onClick={()=>void removeFolder(folder.folderId)}>폴더 제거</button></div>)}</div>}
    ...
    {presets.length>0&&<div className="campaign-option-list">{presets.map((entry)=><div key={entry.entryId}><span><strong>{entry.label}</strong><small>Lv.{entry.pcPreset!.level} · AC {entry.pcPreset!.ac} · HP {entry.pcPreset!.maxHp} · {entry.folderId?folders.find((folder)=>folder.folderId===entry.folderId)?.label??"폴더":"최상위"}</small></span><button type="button" disabled={busy} onClick={()=>void perform(()=>mockAdapter.instantiateCampaignDmLibraryPcPreset(campaign.campaignId,entry.entryId).then(()=>undefined))}>Actor +1</button><button type="button" disabled={busy} onClick={()=>editPreset(entry)}>수정</button><button type="button" disabled={busy} onClick={()=>void deletePreset(entry.entryId)}>삭제</button></div>)}</div>}
