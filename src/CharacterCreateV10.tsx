...
export function CharacterCreateScreenV10({ onDone, onCancel }: { onDone():void; onCancel():void }) {
  ...
  return <div className="focused-create-shell">
    <header className="focused-create-header"><div className="focused-create-title"><span>{draft.editingCharacterId ? "CHARACTER EDIT" : "CHARACTER CREATION"}</span><strong>{draft.name || "이름 없는 캐릭터"}</strong></div><nav className="focused-create-tabs" role="tablist" aria-label="캐릭터 생성 단계">{primary.map((section) => { const state = primaryStatus(plan, section); return <button type="button" role="tab" aria-selected={active.id === section.id} aria-current={active.id === section.id ? "step" : undefined} key={section.id} className={`${active.id === section.id ? "active" : ""} status-${state}`} onClick={() => go(section.id)}><i>{state === "complete" ? "✓" : state === "warning" ? "!" : state === "incomplete" || state === "blocked" ? "•" : ""}</i><span>{LABELS[section.id] ?? section.label}</span></button>; })}</nav><div className="focused-create-header-actions"><button onClick={() => createCharacterDraft("import")}>JSON</button><button onClick={onCancel}>닫기</button></div></header>
