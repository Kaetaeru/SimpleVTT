...
export type SessionCampaignView="overview"|"calendar"|"rations"|"rest";

export function SessionCampaignPane({role,view="overview",onClose,onOpenLevelUp}:{role:"dm"|"player";view?:SessionCampaignView;onClose():void;onOpenLevelUp?(rosterMemberId:string):void}){
  ...
  useEffect(()=>{
    if(view!=="rest"||role!=="dm"||!projection||!api.snapshot?.activeCharacter){setRestPreview(null);setRestPreviewError(null);return;}
    let cancelled=false;
  ...
  },[view,role,projection?.campaignId,projection?.campaignRevision,restAdvanceTime,restConsumeRations,api.snapshot?.activeCharacter.id,api.snapshot?.activeCharacter.hp,api.snapshot?.activeCharacter.tempHp,api.snapshot?.activeCharacter.maxHp]);
  ...
  const viewCopy={overview:{eyebrow:"PARTY",title:"파티 관리",description:"명단 · 경험치 · 레벨업"},calendar:{eyebrow:"WORLD TIME",title:"시간",description:"날짜 · 시각 · 진행"},rations:{eyebrow:"SUPPLIES",title:"식량",description:"잔량 · 소비 · 부족"},rest:{eyebrow:"REST",title:"휴식",description:"회복 · 시간 · 식량"}}[view];

  return <aside className="session-utility-pane session-campaign-pane" aria-label={`세션 ${viewCopy.title}`} data-campaign-view={view}>
    <header className="session-utility-pane-head session-campaign-pane-head"><div><span>{viewCopy.eyebrow}</span><strong>{viewCopy.title}</strong><small>{projection?.campaignName??"연결된 캠페인 없음"} · {viewCopy.description} · {role==="dm"?"DM 조작":"Player 보기"}</small></div><button type="button" autoFocus aria-label={`${viewCopy.title} 닫기`} onClick={onClose}>×</button></header>
    {!projection?<div className="session-campaign-empty"><strong>세션 캠페인이 없습니다.</strong><p>DM이 캠페인에서 세션을 시작하면 {viewCopy.title} 상태가 여기에 표시됩니다.</p></div>:<div className="session-campaign-scroll" data-view={view}>
      ...
      {view==="overview"&&<section className="session-campaign-block" aria-labelledby="session-campaign-roster">
      ...
      {view==="rest"&&role==="dm"&&<section className="session-campaign-block session-campaign-feature-block" aria-labelledby="session-campaign-long-rest">
      ...
      {view==="rest"&&role==="player"&&<ConnectedLongRestPlayerControls/>}

      {view==="calendar"&&<section className="session-campaign-block session-campaign-feature-block" aria-labelledby="session-campaign-calendar">
      ...
      {view==="rations"&&<section className="session-campaign-block session-campaign-feature-block" aria-labelledby="session-campaign-rations">
