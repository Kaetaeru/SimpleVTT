...
import { award, bench, expiredSummonIds, grantItem, handout, sceneConditions, sceneSwitch } from "./handlers/scene";
...
function describe(state:TableState,command:TableCommand):string {
  switch(command.type) {
    ...
    case "handout": return command.image?`자료 공개 · ${command.image.name}`:"자료 공개 해제";
...
export class TableRuntime {
  ...
  private handle(ctx:HandlerContext,command:TableCommand):HandlerResult {
    switch(command.type) {
      ...
      case "handout": return handout(ctx,command);
