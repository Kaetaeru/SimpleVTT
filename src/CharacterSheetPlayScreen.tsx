...
export function CharacterSheetPlayScreen({onScene,onLevelUp,onEdit}:{onScene():void;onLevelUp():void;onEdit():void}) {
  ...
  return <div className="screen sheet-play-screen">
    ...
    <main className="sheet-play-layout">
      ...
      <section className="sheet-play-core">
        <article className="sheet-play-card"><header><div><span className="eyebrow accent">ATTACKS</span><h2>공격 & 피해</h2></div></header><div className="sheet-play-attack-list">{c.attacks.map((attack)=><div key={attack.id}><div><strong>{attack.name}</strong><span>명중 +{attack.bonus}</span><small>{attack.damage}</small></div><button onClick={()=>d20(`${attack.name} 명중`,attack.bonus)}>명중 굴림</button><button onClick={()=>damage(attack.name,attack.damage)}>피해 굴림</button></div>)}{!c.attacks.length&&<p>등록된 공격이 없습니다.</p>}</div></article>
      ...
      <aside className="sheet-play-side">
        <article className="sheet-play-card"><header><div><span className="eyebrow accent">DICE</span><h2>주사위</h2></div></header><div className="sheet-common-dice">{([4,6,8,10,12,20] as DieSides[]).map((sides)=><button key={sides} onClick={()=>rawDie(sides)}>d{sides}</button>)}</div></article>
        ...
        {history.length>0&&<article className="sheet-play-card"><header><div><span className="eyebrow accent">HISTORY</span><h2>최근 굴림</h2></div></header><div className="sheet-roll-history">{history.map((entry)=><button key={entry.id} onClick={()=>setRoll(entry)}><span>{entry.label}</span><strong>{entry.total}</strong></button>)}</div></article>}
      </aside>
    </main>
  </div>;
}
