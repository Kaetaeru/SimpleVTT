...
MockAdapter.prototype.resolveAction=async function resolveOpenHandWholenessAction(actionId:string,targetIds:string[]){
  ...
  const committed=resolveOpenHandWholenessOfBody(SIMPLEVTT_APP_RULES_PROFILE,state,{id:resolutionId,actorId:actor.id,expectedRevision:state.revision,monkLevel:level,subclassId:internal.activeCharacter.subclassIds?.[MONK_OPEN_HAND_CLASS_ID],wisdomModifier,martialArtsDieFace:face});if(committed.status==="rejected")return snapshot;
