...
MockAdapter.prototype.respondToInterrupt=async function respondToQuiveringPalmSeed(accept:boolean,selectedIds?:string[]){
  const internal=this as unknown as AdapterState,resolution=internal.resolution,interrupt=resolution?.interrupt;if(!resolution||interrupt?.id!==SEED_INTERRUPT_ID)return previousRespondToInterrupt.call(this,accept,selectedIds);
