...
import { resolveRollState } from "../domain/profileEngine";
...
type Candidate={followUp:FollowUp;actorId:string;family:"ability-check"|"saving-throw"|"attack-roll";natural:number;initial:number;target:number;handledKey:string;save?:SaveResultVm};
...
function validFollowUp(entry:FollowUp){const modification=entry.modification.mode==="add-die"?Number.isInteger(entry.modification.diceSides)&&entry.modification.diceSides>=2&&entry.modification.diceSides<=20:entry.families.length===1&&entry.families[0]==="saving-throw"&&Number.isFinite(entry.modification.bonus);return Boolean(entry.sourceId)&&(entry.trigger==="failure"||entry.trigger==="after-roll")&&entry.families.some((family)=>family==="ability-check"||family==="saving-throw"||family==="attack-roll")&&modification&&Boolean(entry.payment.resourceId)&&Number.isInteger(entry.payment.amount)&&entry.payment.amount>0;}
...
function eligible(adapter:MockAdapter,internal:AdapterState,ignoreInterrupt=false):Candidate|undefined{
  ...
  if(resolution.rollKind==="attack"){
    ...
    if((ignoreInterrupt?resolution.stage!=="interrupt":resolution.stage!=="attack-result")||resolution.attackOutcome!=="빗나감"||!validD20(natural)||!Number.isFinite(resolution.attackTotal)||!Number.isFinite(resolution.targetAc))return;
    const followUp=followUps(internal.scene,resolution.actorId).find((entry)=>entry.families.includes("attack-roll")&&entry.trigger==="failure"&&!handled.has(`attack-roll:${resolution.actorId}:${entry.sourceId}`)&&hasPayment(adapter,internal,resolution.actorId,entry));
    return followUp?{followUp,actorId:resolution.actorId,family:"attack-roll",natural,initial:resolution.attackTotal!,target:resolution.targetAc!,handledKey:`attack-roll:${resolution.actorId}:${followUp.sourceId}`}:undefined;
  }
  ...
  for(const save of resolution.saveResults){
    ...
    if(followUp?.modification.mode==="reroll"&&!save.modifierContributions)continue;
...
function offer(adapter:MockAdapter,internal:AdapterState){
  ...
  const label=candidate.family==="ability-check"?"능력 판정":candidate.family==="attack-roll"?"공격 굴림":"내성";
...
function restore(resolution:ResolutionView,origin:ActionVm,candidate:Candidate){
  ...
  if(candidate.family==="attack-roll"){resolution.stage="attack-result";resolution.canAdvance=true;resolution.nextLabel=resolution.attackOutcome==="명중"?"피해 굴림":"판정 적용";return;}
...
MockAdapter.prototype.respondToInterrupt=async function respondToD20FollowUp(accept:boolean){
  ...
  const rollId=`${resolution.id}:d20-follow-up:${encodeURIComponent(candidate.actorId)}:${encodeURIComponent(followUp.sourceId)}`;const modification=followUp.modification;const reroll=modification.mode==="reroll";const sides=modification.mode==="add-die"?modification.diceSides:20;const bonus=modification.mode==="reroll"?modification.bonus:0;const rollState=reroll?resolveRollState(SIMPLEVTT_APP_RULES_PROFILE,candidate.save?.rollStateContributions??[]).rollState:"normal";const faces=reroll&&rollState!=="normal"?[rollDie(this,20),rollDie(this,20)]:[rollDie(this,sides)];const face=faces[0];
  const operations:ResolutionOperation[]=[{id:rollId,kind:"d20",actorId:candidate.actorId,request:{family:candidate.family,target:candidate.target,targetSource:`authoritative prior ${candidate.family} target`,modifierContributions:reroll?[...(candidate.save?.modifierContributions??[])]:[{source:`authoritative prior ${candidate.family} modifier total`,value:candidate.initial-candidate.natural}],rollStateContributions:reroll?candidate.save?.rollStateContributions:undefined,rollModifications:reroll?[{source:followUp.presentation.optionName,mode:"reroll",dice:{id:`${rollId}:reroll`,purpose:followUp.sourceId,sides:20,faces}},...(bonus?[{source:followUp.presentation.optionName,mode:"add-flat" as const,value:bonus}]:[])]:[{source:followUp.presentation.optionName,mode:"add-die",dice:{id:`${rollId}:bonus`,purpose:followUp.sourceId,sides,faces}}],dice:{id:`${rollId}:original`,purpose:`authoritative prior ${candidate.family}`,sides:20,faces:[candidate.natural]}}}];
  ...
  }else if(candidate.family==="saving-throw"){
    if(reroll){const index=resolution.saveResults.indexOf(candidate.save!);resolution.authoritativeDice[index]=result.natural;candidate.save!.d20=result.natural;}candidate.save!.total=result.total;candidate.save!.outcome=result.outcome==="success"?"성공":"실패";resolution.compact=`${origin.saveAbility??"내성"} DC ${candidate.save!.dc} · ${resolution.saveResults.filter((entry)=>entry.outcome==="성공").length} 성공 / ${resolution.saveResults.filter((entry)=>entry.outcome==="실패").length} 실패`;appendAdapterInterruptEvents(this,resolution.id,committed.events);restore(resolution,origin,candidate);
  }else{
    resolution.rollModifierContributions=[...(resolution.rollModifierContributions??[]),{source:followUp.sourceId,value:face}];resolution.rollTotal=result.total;resolution.attackTotal=result.total;resolution.attackOutcome=result.outcome==="success"?"명중":"빗나감";resolution.compact=`${candidate.initial} + d${sides} ${face} = ${result.total} vs AC ${candidate.target} — ${resolution.attackOutcome}`;resolution.calculatedOutcome=resolution.compact;resolution.finalOutcome=resolution.compact;appendAdapterInterruptEvents(this,resolution.id,committed.events);restore(resolution,origin,candidate);
  }
