...
function validFollowUp(entry:FollowUp){return Boolean(entry.sourceId)&&(entry.trigger==="failure"||entry.trigger==="after-roll")&&entry.families.some((family)=>family==="ability-check"||family==="saving-throw"||family==="attack-roll")&&entry.modification.mode==="add-die"&&Number.isInteger(entry.modification.diceSides)&&entry.modification.diceSides>=2&&entry.modification.diceSides<=20&&Boolean(entry.payment.resourceId)&&Number.isInteger(entry.payment.amount)&&entry.payment.amount>0;}
