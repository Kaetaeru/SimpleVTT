...
MockAdapter.prototype.respondToInterrupt=async function respondToConsumableD20BonusEffect(accept:boolean,selectedIds?:string[]){
  ...
  const label=effectLabel(effect.metadata);resolution.authoritativeDice.push(face);
  // The attack transaction treats an explicit contribution list as the complete modifier set, so the
  // base attack bonus must be carried alongside the consumed die or the domain re-check drifts
  // ("preview 15 vs domain 8") and the whole Resolution is rejected after the owner accepted.
  const action=Object.values(internal.scene.actionsByActor).flat().find((entry)=>entry.id===resolution.actionId);
  const baseContributions=resolution.rollModifierContributions?.length?resolution.rollModifierContributions:[{source:`action:${resolution.actionId}:attack-bonus`,value:action?.attackBonus??0}];
  resolution.rollModifierContributions=[...baseContributions,{source:effect.sourceId,value:face}];resolution.attackTotal=committed.test.total;resolution.rollTotal=committed.test.total;resolution.attackOutcome=committed.test.outcome==="success"?"명중":"빗나감";resolution.compact=`${committed.test.total-face} + d${sides} ${face} = ${committed.test.total} vs AC ${committed.test.target} — ${resolution.attackOutcome}`;resolution.calculatedOutcome=resolution.compact;resolution.finalOutcome=resolution.compact;resolution.detail.push(`${label} d${sides}: ${face} · 효과 소모`);resolution.stateChanges.push(...projected.stateChanges);resolution.interrupt=undefined;
