...
type HostRecord={
  ...
  outcome?:"declined"|"aborted-complete";
};
  ...
  for(const durable of records){
    ...
    if(durable.phase==="owner-prepared"){
      ...
      if(campaign?.recentRequestIds.includes(durable.preflight.transactionId)){
        current={...durable,phase:"committed",campaignCommitId:connectedLongRestCampaignCommitId(durable.preflight.transactionId)};
      }else{
        current={...durable,phase:"aborted",reason:"Host restarted before the connected Long Rest Campaign global commit"};
      }
      await store.write(current);
    }
    const transaction=stateFromDurable(current);
    hostMap(adapter).set(current.preflight.transactionId,{peer:null,offer:offerFromPreflight(current.preflight),transaction});
...
export function connectedLongRestOwnerPrompts(adapter:MockAdapter):ConnectedLongRestOwnerPrompt[] {
  return [...clientMap(adapter).values()].map((record)=>({
    ...
    phase:record.abortedReason?"aborted":record.materialized?"complete":record.globalCommit?"committed":record.prepared?"prepared":record.decision?.accepted===false?"declined":record.decision?.accepted?"accepted":"offered",
    error:record.abortedReason,
  }));
}

export async function beginConnectedLongRestHostOffer(adapter:MockAdapter,input:{characterId:string;transactionId?:string;advanceMinutes?:number;consumeRations?:boolean}) {
  ...
  const offer:ConnectedLongRestOffer={
    transactionId:required(input.transactionId??transactionId(),"connected Long Rest transaction id"),sessionId:state.sessionId,campaignId:systems.campaignId,campaignRevision:systems.campaignRevision,ownerParticipantId:participantId,
    character:{characterId:mounted.characterId,sourceRevision:mounted.sourceRevision,runtimeRevision:mounted.runtimeRevision},
...
export function receiveConnectedLongRestOwnerOffer(adapter:MockAdapter,offer:ConnectedLongRestOffer) {
  ...
  if(offer.character.sourceRevision!==(app.activeCharacter.sourceRevision??0)||offer.character.runtimeRevision!==(app.activeCharacter.runtimeRevision??0)) throw new Error("connected Long Rest offer Character revision is stale");
  const existing=clientMap(adapter).get(offer.transactionId);
  if(existing){if(!sameOffer(existing.offer,offer)) throw new Error("connected Long Rest transaction changed during retry");return cp(existing.preview);}
  const preview=projectCharacterLongRest(app.activeCharacter);
  clientMap(adapter).set(offer.transactionId,{offer:cp(offer),preview:cp(preview),beforeHp:app.activeCharacter.hp,beforeTempHp:app.activeCharacter.tempHp});
  return preview;
}
...
export function decideConnectedLongRestOwnerOffer(adapter:MockAdapter,transaction:string,accepted:boolean):ConnectedLongRestOwnerDecision {
  ...
  const app=adapter as unknown as {activeCharacter:CharacterSheet};
  if(record.offer.character.characterId!==app.activeCharacter.id||record.offer.character.sourceRevision!==(app.activeCharacter.sourceRevision??0)||record.offer.character.runtimeRevision!==(app.activeCharacter.runtimeRevision??0)) throw new Error("connected Long Rest Character changed before owner decision");
  ...
  const decision:ConnectedLongRestOwnerDecision={transactionId:id,sessionId:record.offer.sessionId,ownerParticipantId:record.offer.ownerParticipantId,character:cp(record.offer.character),accepted};
...
function memoryCharacterStore(document:CharacterLibraryDocumentV1) {
  const store=new MemoryCharacterLibraryStore();store.seed(document.storageRevision,encodeCharacterLibraryV1(document));return store;
}
...
export async function prepareAuthorizedConnectedLongRestOwner(adapter:MockAdapter,preflight:ConnectedLongRestCommitPreflight) {
  ...
  const snapshot=await adapter.getSnapshot();
  const document=getCharacterLibraryPersistenceStateForTests(adapter)?.document;
  if(!document) throw new Error("connected Long Rest owner requires a hydrated Character library");
  let characterStore:CharacterLibraryStore;let preparationStore:ConnectedLongRestOwnerPreparationStore;
  if(isTauriCharacterLibraryRuntime()){characterStore=createPlatformCharacterLibraryStore();preparationStore=new TauriConnectedLongRestOwnerPreparationStore();}
  else{const memory=memoryCharacterStore(document);characterStore=memory;preparationStore=new MemoryConnectedLongRestOwnerPreparationStore(memory);}
  const prepared=await prepareConnectedLongRestOwnerCandidate({preflight,currentDocument:document,currentCharacter:snapshot.activeCharacter,characterStore,preparationStore});
  record.preflight=cp(preflight);record.prepared=cp(prepared.prepared);record.characterStore=characterStore;record.preparationStore=preparationStore;
...
export async function recordConnectedLongRestHostOwnerPrepared(adapter:MockAdapter,peer:string,prepared:ConnectedLongRestOwnerPrepared) {
  ...
  if(record.transaction.phase==="committed"||record.transaction.phase==="complete") return {status:"committed" as const,peer,commit:{transactionId:record.transaction.preflight.transactionId,campaignCommitId:record.transaction.campaignCommitId,ownerParticipantId:record.transaction.preflight.ownerParticipantId,character:cp(record.transaction.preflight.character),preparationId:record.transaction.preparationId},snapshot:await adapter.getSnapshot()};
  if(record.transaction.phase==="aborted") return {status:"aborted" as const,peer,transactionId:prepared.transactionId,reason:record.transaction.reason};
  const ownerPrepared=record.transaction.phase==="owner-prepared"?record.transaction:recordConnectedLongRestOwnerPrepared(record.transaction,prepared);
  record.transaction=ownerPrepared;
  const store=hostCoordinatorStore(adapter);
  try{await store.write(durableRecord(ownerPrepared));}
  ...
  try{
    ...
    return {status:"committed" as const,peer,commit,snapshot:campaign.snapshot,projectionWarning:[campaign.projectionWarning,persistenceWarning].filter(Boolean).join(" · ")||undefined};
  }catch(error){const reason=error instanceof Error?error.message:String(error);record.transaction=abortConnectedLongRestTransaction(record.transaction,reason);await store.write(durableRecord(record.transaction)).catch(()=>undefined);return {status:"aborted" as const,peer,transactionId:prepared.transactionId,reason};}
}
...
export async function materializeConnectedLongRestOwnerAfterGlobalCommit(adapter:MockAdapter,commit:ConnectedLongRestGlobalCommit) {
  ...
  if(!record?.prepared||!record.preflight||!record.characterStore||!record.preparationStore) throw new Error("connected Long Rest owner preparation is missing for global commit");
  if(record.globalCommit&&record.globalCommit.campaignCommitId!==commit.campaignCommitId) throw new Error("connected Long Rest global commit identity changed during retry");
  record.globalCommit=cp(commit);
  if(record.materialized&&record.projection) return {materialized:cp(record.materialized),projection:cp(record.projection),snapshot:await adapter.getSnapshot()};
  ...
  const snapshot=await adapter.getSnapshot();const projection=buildCharacterSessionProjectionV1(snapshot.activeCharacter,snapshot.catalog);
  const materialized:ConnectedLongRestOwnerMaterialized={transactionId:record.prepared.transactionId,ownerParticipantId:record.prepared.ownerParticipantId,character:cp(record.prepared.character),preparationId:record.prepared.preparationId};
  record.materialized=cp(materialized);record.projection=cp(projection);return {materialized,projection,snapshot};
}

export async function abortConnectedLongRestOwner(adapter:MockAdapter,transaction:string,reason:string) {
  const id=required(transaction,"connected Long Rest transaction id");const record=clientMap(adapter).get(id);if(!record) return false;
  ...
  record.abortedReason=required(reason,"connected Long Rest abort reason");if(record.prepared&&record.preparationStore) await abortConnectedLongRestOwnerCandidate(record.preparationStore,record.prepared);return true;
}

export async function completeConnectedLongRestHostOwnerAbort(adapter:MockAdapter,peer:string,aborted:ConnectedLongRestOwnerAborted) {
  ...
  record.outcome="aborted-complete";
  return {status:"complete" as const,transactionId:aborted.transactionId};
}

export async function completeConnectedLongRestHostOwnerMaterialization(adapter:MockAdapter,peer:string,materialized:ConnectedLongRestOwnerMaterialized,projection:CharacterSessionProjectionV1) {
  ...
  if(record.transaction.phase!=="committed"){if(record.transaction.phase==="complete") return cp(record.transaction);throw new Error(`connected Long Rest owner materialization is invalid during ${record.transaction.phase}`);}
  const state=connectedStateFor(adapter);const manifest=state.peerManifests.get(peer);if(!manifest?.character) throw new Error("connected Long Rest owner materialization requires the accepted Character manifest");
  if(projection.characterId!==materialized.character.characterId) throw new Error("connected Long Rest materialized projection Character mismatch");if(projection.sourceRevision!==materialized.character.sourceRevision) throw new Error("connected Long Rest materialized projection source revision changed");if(projection.runtimeRevision<=materialized.character.runtimeRevision) throw new Error("connected Long Rest materialized projection did not advance runtime revision");
  const app=adapter as unknown as {catalog:AppSnapshot["catalog"]};const reconstructed=reconstructCharacterSessionProjectionV1(projection,app.catalog);if(reconstructed.status==="rejected") throw new Error(reconstructed.error);const refreshed=refreshReconstructedCharacterSessionProjection(adapter,peer,reconstructed);if(refreshed.status==="rejected") throw new Error(refreshed.error);
  record.transaction=recordConnectedLongRestOwnerMaterialized(record.transaction,materialized);state.peerManifests.set(peer,{...manifest,character:{characterId:projection.characterId,sourceRevision:projection.sourceRevision,runtimeRevision:projection.runtimeRevision}});await hostCoordinatorStore(adapter).delete(materialized.transactionId).catch(()=>undefined);return cp(record.transaction);
}
...
export function connectedLongRestHostRecoveryMessages(adapter:MockAdapter,peer:string):ConnectedLongRestWireMessage[] {
  ...
  for(const record of hostMap(adapter).values()){
    if(record.outcome||!peerOwnsRecoveredRecord(adapter,peer,record)) continue;
    if(!record.transaction){messages.push({type:"long-rest-offer",offer:cp(record.offer)});continue;}
    if(record.transaction.phase==="approved") messages.push({type:"long-rest-prepare-authorized",preflight:cp(record.transaction.preflight)});
    else if(record.transaction.phase==="committed") messages.push({type:"long-rest-global-commit",commit:{transactionId:record.transaction.preflight.transactionId,campaignCommitId:record.transaction.campaignCommitId,ownerParticipantId:record.transaction.preflight.ownerParticipantId,character:cp(record.transaction.preflight.character),preparationId:record.transaction.preparationId}});
    else if(record.transaction.phase==="aborted") messages.push({type:"long-rest-abort",transactionId:record.transaction.preflight.transactionId,reason:record.transaction.reason,...(record.transaction.preparationId?{ownerParticipantId:record.transaction.preflight.ownerParticipantId,character:cp(record.transaction.preflight.character),preparationId:record.transaction.preparationId}:{})});
  }
  return messages;
}
...
export function connectedLongRestClientRecoveryMessages(adapter:MockAdapter):ConnectedLongRestWireMessage[] {
  ...
  for(const record of clientMap(adapter).values()){
    ...
    if(record.materialized&&record.projection) messages.push({type:"long-rest-owner-materialized",materialized:cp(record.materialized),projection:cp(record.projection)});
    else if(record.prepared) messages.push({type:"long-rest-owner-prepared",prepared:cp(record.prepared)});
    else if(record.decision) messages.push({type:"long-rest-decision",decision:cp(record.decision)});
  }
  return messages;
}
