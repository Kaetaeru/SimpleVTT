...
const pendingStashDeposits=new WeakMap<MockAdapter,Map<string,{resolve():void;reject(error:Error):void;timer:ReturnType<typeof setTimeout>}>>();
...
export function decodeCampaignSystemsEnvelope(raw:string):CampaignSystemsEnvelope|null{
  ...
  if(!Array.isArray(projection.roster)||projection.roster.some((value)=>{const member=object(value);return typeof member?.rosterMemberId!=="string"||typeof member.label!=="string"||typeof member.kind!=="string"||typeof member.active!=="boolean"||(member.presentInSession!==undefined&&typeof member.presentInSession!=="boolean");})) return null;
...
async function sendOwnerInventoryResult(client:MockAdapter,peer:string,request:CampaignOwnerInventoryRequest){
  ...
  try{
    ...
    // This mutation is initiated by a transport message, not an AppProvider
    // command. Publish it explicitly so the owning Client renders the durable
    // Character change immediately (GP, items, and derived actions).
...
async function onMessageWithCampaignSystems(handler:(message:SessionTransportMessage)=>void){
  ...
  return baseOnMessage((message)=>{
    ...
    if(stashRequest){
      if(host)void (async()=>{
        ...
        try{
          ...
          const member=campaign?.roster.find((entry)=>entry.characterId===manifest.characterId);
          if(!member||!(member.stashPermission==="request"||member.stashPermission==="manage"))throw new Error("파티 보관함 입고 권한이 없습니다.");
          if(campaign?.partyStash.policy==="dm-managed")throw new Error("이 파티 보관함은 DM 전용 관리 정책입니다.");
          if(campaign?.partyStash.policy==="dm-approval"&&stashRequest.command.direction==="stash-to-character")throw new Error("이 파티 보관함의 출고는 DM 승인이 필요합니다.");
          // The Host owns the complete transaction. It asks the owning Client to
          // mutate its durable Character, refreshes the Host-side owner
          // projection, and only then commits the Campaign Stash delta. Keeping
          // the debit on the Client before this request allowed the two durable
          // stores to drift (and made a deposit look like minted currency).
          await host.transferPartyStash(stashRequest.command);
          // Transport-initiated mutations bypass AppProvider.apply(), so publish
          // the final Host snapshot explicitly after both halves have settled.
          await publishConnectedSnapshot(host);
        }catch(cause){error=cause instanceof Error?cause.message:String(cause);}
    ...
    if(client&&stashResult){
      ...
      if(stashResult.accepted)pending.resolve();else pending.reject(new Error(stashResult.error||"DM이 파티 보관함 이동을 거절했습니다."));
      return;
    }
...
MockAdapter.prototype.serveCampaignMeals=broadcastAfter(MockAdapter.prototype.serveCampaignMeals);
MockAdapter.prototype.setCampaignMemberMeals=broadcastAfter(MockAdapter.prototype.setCampaignMemberMeals);
MockAdapter.prototype.undoCampaignMeal=broadcastAfter(MockAdapter.prototype.undoCampaignMeal);
...
MockAdapter.prototype.transferPartyStash=async function transferConnectedPartyStash(command){
  ...
  const waitForHost=new Promise<void>((resolve,reject)=>{
    ...
    const timer=setTimeout(()=>{map.delete(command.requestId);reject(new Error("파티 보관함 이동 응답 시간이 초과되었습니다."));},8000);
    map.set(command.requestId,{resolve,reject,timer});pendingStashDeposits.set(this,map);
  });
  try{await tauriSessionTransport.send(JSON.stringify({type:"campaign-stash-deposit",sessionId:state.sessionId,command} satisfies CampaignStashDepositRequest));await waitForHost;}
  catch(error){
    const pending=pendingStashDeposits.get(this)?.get(command.requestId);if(pending){clearTimeout(pending.timer);pendingStashDeposits.get(this)?.delete(command.requestId);}
    throw error;
  }
  return this.getSnapshot();
};
