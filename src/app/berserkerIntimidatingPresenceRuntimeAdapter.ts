...
function projectAction(internal:AdapterState,snapshot:AppSnapshot):ActionVm|undefined {
  const character=snapshot.activeCharacter;
  const level=barbarianLevel(character);
  if(level<14||character.subclassIds?.[BARBARIAN_CLASS_ID]!==BARBARIAN_BERSERKER_SUBCLASS_ID)return undefined;
  ...
  return {
    ...
    summary:`지혜 내성 DC ${saveDc} · 실패 시 1분 공포 · ${resource.current}/${resource.max}`,
    available,
...
MockAdapter.prototype.resolveAction=async function resolveBerserkerIntimidatingPresenceAction(actionId:string,targetIds:string[]){
  ...
  const actor=internal.scene.entities.find((entry)=>entry.id===snapshot.activeCharacter.id);
  ...
  try {
    const targets=targetIds.map((targetId,index)=>{
      ...
      return {
        ...
        saveDice:{id:`${resolutionId}:save:${targetId}`,purpose:"Intimidating Presence Wisdom save",sides:20 as const,faces:[(MockAdapter.prototype as unknown as DicePrototype).d20.call(this,actionId,index)]},
      };
    });
    ...
    const resolution:ResolutionView={
      ...
      saveResults:[],
      damageComponents:[],
      compact:`대상 ${targetIds.length}명 위압 판정 완료`,
      detail:["지혜 내성 실패 대상은 1분 동안 공포 상태가 됩니다."],
      provenance:["SRD 5.2.1 · Path of the Berserker · Intimidating Presence"],
      calculatedOutcome:"위압적인 존재감 적용",
      finalOutcome:"위압적인 존재감 적용",
