...
import {
  buildProgressionPlanPhase08RogueThief,
  resolveProgressionPhase08RogueThief,
  thiefSyntheticSelectionsForPlan,
} from "../domain/progressionPhase08RogueThief";
import {
  ROGUE_THIEF_CLASS_ID,
  THIEF_SUPREME_SNEAK_FEATURE_ID,
  THIEF_THIEFS_REFLEXES_FEATURE_ID,
  THIEF_USE_MAGIC_DEVICE_FEATURE_ID,
} from "../domain/rogueThief";
import { ROGUE_THIEF_SUBCLASS_ID } from "../domain/srdSubclassCatalog";
...
const THIEF_FEATURE_LABELS:Record<string,string> = {
  [THIEF_SUPREME_SNEAK_FEATURE_ID]:"최고의 은신",
  [THIEF_USE_MAGIC_DEVICE_FEATURE_ID]:"마법 장치 사용",
  [THIEF_THIEFS_REFLEXES_FEATURE_ID]:"도둑의 반사 신경",
};
...
function syncDraftPlan(draft:LevelUpDraft,plan:ReturnType<typeof buildProgressionPlanPhase08RogueThief>) {
  draft.validation = [
...
function thiefRelevant(result:WizardEvocationProgressionState,plan:ReturnType<typeof buildProgressionPlanPhase08RogueThief>) {
  return plan.targetClassId === ROGUE_THIEF_CLASS_ID
    && result.subclassIds?.[ROGUE_THIEF_CLASS_ID] === ROGUE_THIEF_SUBCLASS_ID;
}

const oldGetSnapshot = MockAdapter.prototype.getSnapshot;
const oldCommitLevelUp = MockAdapter.prototype.commitLevelUp;

MockAdapter.prototype.getSnapshot = async function getSnapshotWithRogueThiefProgression() {
  const internal = this as unknown as AdapterState;
  const snapshot = await oldGetSnapshot.call(this);
  if (!internal.levelUpDraft || !snapshot.levelUpDraft) return snapshot;
  const plan = buildProgressionPlanPhase08RogueThief(characterState(internal.activeCharacter),requestFor(internal));
...
MockAdapter.prototype.commitLevelUp = async function commitLevelUpWithRogueThiefProgression() {
  const internal = this as unknown as AdapterState;
  ...
  const resolved = resolveProgressionPhase08RogueThief(stateBefore,request);
  if (resolved.status === "rejected") {
    syncDraftPlan(internal.levelUpDraft,resolved.plan);
    return internal.getSnapshot();
  }
  if (!thiefRelevant(resolved.state as WizardEvocationProgressionState,resolved.plan)) return oldCommitLevelUp.call(this);

  const originalSelections = clone(internal.levelUpDraft.progressionSelections ?? {});
  internal.levelUpDraft.progressionSelections = clone(thiefSyntheticSelectionsForPlan(stateBefore,request,resolved.plan));
  ...
  internal.activeCharacter.subclassSources[ROGUE_THIEF_CLASS_ID] ??= "SRD 5.2.1 · 로그 · 도둑";
  const labels = (next.subclassFeatureIds ?? [])
    .map((id) => THIEF_FEATURE_LABELS[id])
  ...
  if (activity) {
    ...
    activity.stateChanges = [...activity.stateChanges,"Phase 08 Thief → Character Revision"];
  }
  return internal.getSnapshot();
};
