...
function portableMechanics(value:unknown,label:string):InstalledCommonPlayMechanicV1[] {
  ...
  return value.map((item,index)=>{
    ...
    if (mechanic.kind!=="common-play") throw new Error(`${label} cannot be activated by the generic Catalog: unsupported mechanic kind ${String(mechanic.kind)}`);
    const id=mechanic.id===undefined?undefined:string(mechanic.id,`${label}[${index}].id`);
    const config=parseCommonPlayOperationDefinition(mechanic.config,`${label}[${index}].config`);
    return {kind:"common-play",config:cp(config),...(id?{id}:{})};
  });
}
