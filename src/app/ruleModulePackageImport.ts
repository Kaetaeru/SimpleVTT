...
import { normalizeInstalledCommonPlayMechanics } from "./portableCommonPlayMechanics";
import type {
...
export function parseRuleModulePackage(payload:string):ParsedRuleModulePackage {
  ...
  const entries=raw.content.map((item,index)=>{
    ...
    const mechanics=normalizeInstalledCommonPlayMechanics(value.mechanics,`content[${index}].mechanics`);
