...
} from "../domain/progressionPhase08WeaponMastery";
import type { WeaponMasteryProgressionState } from "../domain/weaponMasteryProgression";
...
function characterState(sheet:CharacterSheet):WeaponMasteryProgressionState {
...
function syncDraftPlan(draft:LevelUpDraft,plan:ReturnType<typeof buildProgressionPlanPhase08WeaponMastery>) {
  draft.validation = [
...
MockAdapter.prototype.getSnapshot = async function getSnapshotWithWeaponMasteryProgression() {
  const internal = this as unknown as AdapterState;
  const snapshot = await oldGetSnapshot.call(this);
  if (!internal.levelUpDraft || !snapshot.levelUpDraft) return snapshot;
  const plan = buildProgressionPlanPhase08WeaponMastery(characterState(internal.activeCharacter),requestFor(internal));
...
MockAdapter.prototype.commitLevelUp = async function commitLevelUpWithWeaponMasteryProgression() {
  const internal = this as unknown as AdapterState;
  ...
  const resolved = resolveProgressionPhase08WeaponMastery(stateBefore,request);
  if (resolved.status === "rejected") {
    syncDraftPlan(internal.levelUpDraft,resolved.plan);
    return internal.getSnapshot();
  }
  const masteryDiff = resolved.plan.diffs.find((entry) => entry.label === "무기 통달");
  const snapshot = await oldCommitLevelUp.call(this);
  if (internal.levelUpDraft || !masteryDiff) return snapshot;
  const next = resolved.state as WeaponMasteryProgressionState;
  internal.activeCharacter.weaponMasteryIds = clone(next.weaponMasteryIds ?? []);
  internal.activeCharacter.weaponMasterySources = clone(next.weaponMasterySources ?? {});
  internal.syncChar();
  const activity = internal.activity[0];
  if (activity) {
    activity.detail = [...activity.detail,`무기 통달: ${masteryDiff.before} → ${masteryDiff.after} (${masteryDiff.source})`];
    activity.stateChanges = [...activity.stateChanges,"Phase 08 Weapon Mastery → Character Revision"];
  }
  return internal.getSnapshot();
};
