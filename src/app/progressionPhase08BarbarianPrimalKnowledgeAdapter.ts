...
} from "../domain/progressionPhase08BarbarianPrimalKnowledge";
...
function syncDraftPlan(draft:LevelUpDraft,plan:ReturnType<typeof buildProgressionPlanPhase08BarbarianPrimalKnowledge>) {
  draft.validation = [
...
MockAdapter.prototype.getSnapshot = async function getSnapshotWithBarbarianPrimalKnowledge() {
  const internal = this as unknown as AdapterState;
  const snapshot = await oldGetSnapshot.call(this);
  if (!internal.levelUpDraft || !snapshot.levelUpDraft) return snapshot;
  const plan = buildProgressionPlanPhase08BarbarianPrimalKnowledge(characterState(internal.activeCharacter),requestFor(internal));
...
MockAdapter.prototype.commitLevelUp = async function commitLevelUpWithBarbarianPrimalKnowledge() {
  const internal = this as unknown as AdapterState;
  ...
  const resolved = resolveProgressionPhase08BarbarianPrimalKnowledge(stateBefore,request);
  if (resolved.status === "rejected") {
    syncDraftPlan(internal.levelUpDraft,resolved.plan);
    return internal.getSnapshot();
  }
  const primalDiff = resolved.plan.diffs.find((entry) => entry.label === "원초적 지식 · 기술 숙련");
  const snapshot = await oldCommitLevelUp.call(this);
  if (internal.levelUpDraft || !primalDiff) return snapshot;
  internal.activeCharacter.skills = clone(resolved.state.proficientSkills ?? []);
  ...
  if (activity) {
    activity.detail = [...activity.detail,`원초적 지식: ${primalDiff.after} 기술 숙련 (${primalDiff.source})`];
    activity.stateChanges = [...activity.stateChanges,"Phase 08 Barbarian Primal Knowledge → Character Revision"];
  }
  return internal.getSnapshot();
};
