...
import {
  ...
  evocationSyntheticSelectionsForPlan,
  resolveProgressionPhase08WizardEvocation,
} from "../domain/progressionPhase08WizardEvocation";
import {
...
} from "../domain/wizardEvocationProgression";

const clone = <T,>(value:T):T => structuredClone(value);
const unique = (values:string[]) => [...new Set(values.filter(Boolean))];
const normalizedSkillName = (value:string) => value.replace(/\s+[+-]\d+$/,"").trim();

const EVOCATION_FEATURE_LABELS:Record<string,string> = {
  [EVOCATION_SAVANT_FEATURE_ID]:"환기술 전문가",
  [POTENT_CANTRIP_FEATURE_ID]:"강력한 소마법",
  [SCULPT_SPELLS_FEATURE_ID]:"주문 조형",
  [EMPOWERED_EVOCATION_FEATURE_ID]:"강화된 환기술",
  [OVERCHANNEL_FEATURE_ID]:"과충전",
};
...
function syncDraftPlan(draft:LevelUpDraft,plan:ReturnType<typeof buildProgressionPlanPhase08WizardEvocation>) {
  draft.validation = [
...
function evocationRelevant(result:WizardEvocationProgressionState,plan:ReturnType<typeof buildProgressionPlanPhase08WizardEvocation>) {
  return plan.targetClassId === WIZARD_EVOCATION_CLASS_ID
    && result.subclassIds?.[WIZARD_EVOCATION_CLASS_ID] === WIZARD_EVOCATION_SUBCLASS_ID;
}

const oldGetSnapshot = MockAdapter.prototype.getSnapshot;
const oldCommitLevelUp = MockAdapter.prototype.commitLevelUp;

MockAdapter.prototype.getSnapshot = async function getSnapshotWithWizardEvocationProgression() {
  const internal = this as unknown as AdapterState;
  const snapshot = await oldGetSnapshot.call(this);
  if (!internal.levelUpDraft || !snapshot.levelUpDraft) return snapshot;
  const plan = buildProgressionPlanPhase08WizardEvocation(characterState(internal.activeCharacter),requestFor(internal));
...
MockAdapter.prototype.commitLevelUp = async function commitLevelUpWithWizardEvocationProgression() {
  const internal = this as unknown as AdapterState;
  ...
  const resolved = resolveProgressionPhase08WizardEvocation(stateBefore,request);
  if (resolved.status === "rejected") {
    syncDraftPlan(internal.levelUpDraft,resolved.plan);
    return internal.getSnapshot();
  }
  if (!evocationRelevant(resolved.state as WizardEvocationProgressionState,resolved.plan)) return oldCommitLevelUp.call(this);

  const originalSelections = clone(internal.levelUpDraft.progressionSelections ?? {});
  internal.levelUpDraft.progressionSelections = clone(evocationSyntheticSelectionsForPlan(stateBefore,request,resolved.plan));
  ...
  internal.activeCharacter.spellbookSpells = clone(next.spellbookSpellIds ?? internal.activeCharacter.spellbookSpells ?? []);
  internal.activeCharacter.spellbookSpellSources = clone(next.spellbookSpellSources ?? internal.activeCharacter.spellbookSpellSources ?? {});
  ...
  internal.activeCharacter.subclassSources[WIZARD_EVOCATION_CLASS_ID] ??= "SRD 5.2.1 · 위저드 · 환기 학파";
  const labels = (next.subclassFeatureIds ?? [])
    .map((id) => EVOCATION_FEATURE_LABELS[id])
  ...
  if (activity) {
    const details = resolved.plan.diffs.filter((diff) => diff.label.includes("환기술 전문가") || diff.label === "서브클래스 특성");
    activity.detail = [...activity.detail,...details.map((diff) => `${diff.label}: ${diff.after} (${diff.source})`)];
    activity.stateChanges = [...activity.stateChanges,"Phase 08 School of Evocation → Character Revision"];
  }
  return internal.getSnapshot();
};
