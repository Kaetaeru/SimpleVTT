...
const MANAGED = /종족을 선택|배경을 선택|클래스를 선택|전투 방식 Choice|주문 목록은 아직|기술 숙련|semantic map|카탈로그 ChoiceDefinition/;
...
export function validate(d: CharacterCreateDraft): ValidationMessage[] {
  ...
  if ((SPELLS[id]?.length ?? 0) > 0) out.push({ severity: "warning", message: "주문 목록은 아직 SRD class spell-list mapping 전이라 DEMO fallback을 표시합니다." });
...
export function buildPlan(d: CharacterCreateDraft): CharacterCreationPlan {
  ...
  const sections = [
    ...
    mk("species", "species", "종족", "SRD catalog의 종족을 독립적으로 선택합니다.", st(true, false, !!d.species), true, ["rules"], choose(SPECIES, d.species), d.species ? SPECIES.find((x) => x.name === d.species)?.grants ?? [] : []),
    mk("background", "background", "배경", "SRD catalog의 배경 source를 선택합니다.", st(true, false, !!d.background), true, ["rules"], choose(BACKGROUNDS, d.background), d.background ? BACKGROUNDS.find((x) => x.name === d.background)?.grants ?? [] : []),
    mk("class", "class", "클래스", "12개 SRD 클래스 정의와 시작 레벨 metadata를 catalog에서 읽습니다.", st(true, false, !!d.className), true, ["rules"], choose(CLASSES, d.className), d.className ? CLASSES.find((x) => x.name === d.className)?.grants ?? [] : []),
    ...
    mk("spells", "spells", "주문 · 기타 선택", "주문 자체 catalog는 존재하지만 class spell-list 연결이 아직 완성 전이라 데모 fallback을 명시해 표시합니다.", spells.length ? "warning" : "not-applicable", spells.length > 0, ["class"], choose(spells, d.selectedSpells), spells.length ? ["DEMO fallback · Phase 08 class spell-list mapping pending"] : [], /주문 목록/),
