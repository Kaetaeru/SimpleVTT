...
} from "../domain/progressionPhase08Warlock";
...
function syncDraftPlan(draft: LevelUpDraft, plan: ReturnType<typeof buildProgressionPlanPhase08Warlock>) {
  draft.validation = [
...
MockAdapter.prototype.getSnapshot = async function getSnapshotWithWarlockReplacement() {
  const internal = this as unknown as AdapterState;
  const snapshot = await oldGetSnapshot.call(this);
  if (!internal.levelUpDraft || !snapshot.levelUpDraft) return snapshot;
  const plan = buildProgressionPlanPhase08Warlock(characterState(internal.activeCharacter), requestFor(internal));
...
MockAdapter.prototype.commitLevelUp = async function commitLevelUpWithWarlockReplacement() {
  const internal = this as unknown as AdapterState;
  ...
  const resolved = resolveProgressionPhase08Warlock(stateBefore, request);
  if (resolved.status === "rejected") {
    syncDraftPlan(internal.levelUpDraft, resolved.plan);
    return internal.getSnapshot();
  }

  const replacementDiff = resolved.plan.diffs.find((diff) => diff.label === "섬뜩한 기원술 교체");
  const snapshot = await oldCommitLevelUp.call(this);
  if (internal.levelUpDraft || !replacementDiff) return snapshot;

  internal.activeCharacter.eldritchInvocationIds = clone(resolved.state.eldritchInvocationIds ?? []);
  internal.activeCharacter.eldritchInvocationSources = clone(resolved.state.eldritchInvocationSources ?? {});
  ...
  if (activity) {
    activity.detail = [...activity.detail, `섬뜩한 기원술 교체: ${replacementDiff.before} → ${replacementDiff.after} (${replacementDiff.source})`];
    activity.stateChanges = [...activity.stateChanges, "Phase 08 Warlock invocation replacement → Character Revision"];
  }
