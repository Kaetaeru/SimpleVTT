...
export async function applyAdapterRuntimeEffectApplication(
  ...
  if (!commitAdapterTurnRuntimeState(adapter,internal.scene,input.revision,resolved.state)) {
    return { status:"rejected",error:"turn runtime revision changed before effect application commit" };
  }

  const events=resolved.events.map((event)=>structuredClone(event));
  ...
  return { status:"committed",events,activity,revision:resolved.state.revision };
}
