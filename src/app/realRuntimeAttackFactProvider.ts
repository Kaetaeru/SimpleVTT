...
export function resolveRuntimeAttackFact(action:ActionVm,fixedFaces:number[]):Phase09AttackFact {
  ...
  if (!actionDamage) throw new Error(`runtime weapon attack is missing structured ActionVm damage: ${action.id}`);
  if (actionDamage.dice !== String(weapon.damage)) {
    throw new Error(`runtime/canonical weapon damage drift: ${action.id} ${actionDamage.dice} != ${weapon.damage}`);
  }
  if (fixedFaces.length < formula.count * 2) {
    throw new Error(`runtime attack fixture requires ${formula.count * 2} fixed faces for critical replay: ${action.id}`);
  }
  return {
    sourceKind:"weapon",
...
export function resolveRuntimeTargetingFact(target:SceneEntity):Phase09TargetingFact {
  ...
  if (!Number.isFinite(distanceFeet) || distanceFeet < 0) {
    throw new Error(`missing structured runtime distance for target: ${target.id}`);
  }
  return {
    distanceFeet,
    visible:true,
