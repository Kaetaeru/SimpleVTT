...
interface PendingSpatialMembershipFact {
  actionId:string;
...
const pendingFacts=new WeakMap<MockAdapter,PendingSpatialMembershipFact>();
...
function spatialFactActionId(artifactId:string) {
  return `common-play-zone-spatial-membership:${encodeURIComponent(artifactId)}:${Date.now()}:${Math.floor(Math.random()*1000)}`;
}
...
export async function submitAuthoritativeSpatialZoneMembershipFact(
  ...
  const actionId=spatialFactActionId(fact.artifactId);
