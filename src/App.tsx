...
function CharacterLibraryScreen({ onOpen, onCreate }: { onOpen(): void; onCreate(): void }) {
  const { snapshot, createCharacterDraft, duplicateCharacterDraft, deleteCharacter } = useSimpleVtt();
  ...
  return <div className="screen page-dark">
    ...
    <div className="character-library-grid">
      ...
      {snapshot.characters.map((character) => <article key={character.id} className="character-card-entry"><button className="character-card" onClick={onOpen}><div className="character-card-portrait">{character.name.slice(0, 1)}</div><div><span className="badge">{character.saveState === "saved" ? "저장됨" : "초안"}</span><h2>{character.name}</h2><p>{character.className} {character.level} · {character.species} · {character.background}</p><div className="metric-strip"><span>HP <b>{character.hp}/{character.maxHp}</b></span><span>AC <b>{character.ac}</b></span></div></div></button><div className="character-card-actions"><button onClick={async()=>{await duplicateCharacterDraft(character.id);onCreate();}}>복제</button><button className="danger-action" disabled={snapshot.characters.length<=1} onClick={()=>{if(window.confirm(`${character.name}을(를) 삭제할까요?`))void deleteCharacter(character.id);}}>삭제</button></div></article>)}
