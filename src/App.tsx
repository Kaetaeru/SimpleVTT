...
export function App() {
  ...
  const playerNav: Array<[AppRoute, string, string]> = [
    ["characters", "내 캐릭터", "◉"], ["scene", "현재 장면", "◆"], ["catalog", "규칙 카탈로그", "▤"], ["activity", "활동 기록", "≡"], ["session", "세션", "⌁"], ["settings", "설정", "⚙"],
  ];
  const dmNav: Array<[AppRoute, string, string]> = [
    ["scene", "세션", "◆"], ["combatants", "컴배턴트", "♜"], ["catalog", "규칙 카탈로그", "▤"], ["activity", "활동 기록", "≡"], ["session", "연결", "⌁"], ["settings", "설정", "⚙"],
  ];
    ...
    <div className="app-shell">
      ...
      <section className="workspace">
        <header className="topbar">
          <div><span className="eyebrow">{snapshot.role === "player" ? "플레이어" : "던전 마스터"}</span><strong>{topTitle(route, snapshot.role)}</strong></div>
          <div className="topbar-meta"><span>{snapshot.sessionMode === "initiative" ? `이니셔티브 · ${snapshot.scene.round}라운드` : "자유 진행"}</span><span className={`status-text ${snapshot.connectionState}`}>{connectionLabel(snapshot.connectionState)}</span></div>
        </header>
...
function topTitle(route: AppRoute, role: "player" | "dm") {
  ...
  if (route === "scene") return role === "player" ? "현재 장면" : "DM 세션";
...
function EdgeBanner() {
  ...
  return <div className={`edge-banner ${snapshot.edgeState}`}><strong>{snapshot.edgeState === "save-error" ? "저장 실패 상태" : "지원되지 않는 상태"}</strong><span>{snapshot.edgeState === "save-error" ? "다음 Character Revision 저장을 실패시키는 Reference 상태입니다." : "현재 RulesProfile/Capability로 표현할 수 없는 콘텐츠 상태 예시입니다."}</span></div>;
}
...
function PlayerSceneScreen() {
  ...
  return <div className="screen scene-screen"><ScreenHead kicker="현재 장면" title={scene.name} description={snapshot.sessionMode === "initiative" ? `${scene.round}라운드 · ${scene.entities.find((entity) => entity.id === scene.currentActorId)?.name}의 턴` : "자유 진행 · 턴/행동경제 없이 장면과 Persistent Resource 중심"} actions={canEndTurn ? <button className="primary" onClick={() => endTurn()}>턴 종료</button> : undefined}/><div className="scene-layout"><aside className="scene-side"><PanelTitle>{snapshot.sessionMode === "initiative" ? "이니셔티브" : "장면 참가자"}</PanelTitle><EntityList entities={scene.entities} selectedAction={targeting.selectedAction} selectedTargetIds={targeting.selectedTargetIds} onTarget={targeting.chooseTarget} onHover={(id) => setUiDebug({ hoverTargetId: id })}/><PanelTitle>파티</PanelTitle><EntityList entities={allies} selectedAction={targeting.selectedAction} selectedTargetIds={targeting.selectedTargetIds} onTarget={targeting.chooseTarget} onHover={(id) => setUiDebug({ hoverTargetId: id })} compact/></aside><section className="scene-center"><div className="scene-stage"><div className="formation enemies">{enemies.map((entity) => <EntityPortrait key={entity.id} entity={entity} selectedAction={targeting.selectedAction} selectedTargetIds={targeting.selectedTargetIds} onTarget={targeting.chooseTarget} onHover={(id) => setUiDebug({ hoverTargetId: id })}/>)}</div><div className="scene-context"><strong>{scene.name}</strong><span>장면 일러스트 영역 · 전술 격자 아님</span>{targeting.selectedAction && <em>{targeting.selectedAction.name} · 유효한 대상을 직접 클릭</em>}</div><div className="formation allies">{allies.map((entity) => <EntityPortrait key={entity.id} entity={entity} selectedAction={targeting.selectedAction} selectedTargetIds={targeting.selectedTargetIds} onTarget={targeting.chooseTarget} onHover={(id) => setUiDebug({ hoverTargetId: id })}/>)}</div></div><ActionConsole actor={actor} actions={actions} economy={scene.economyByActor[actor.id]} resources={snapshot.activeCharacter.resources.map((resource) => `${resource.label} ${resource.current}/${resource.max}`)} filter={filter} setFilter={setFilter} selectedActionId={targeting.selectedActionId} selectedTargetIds={targeting.selectedTargetIds} onAction={targeting.chooseAction} onCancel={targeting.cancel} onCompleteMulti={targeting.completeMulti} sessionMode={snapshot.sessionMode}/></section><aside className="scene-side"><PanelTitle>적</PanelTitle><EntityList entities={enemies} selectedAction={targeting.selectedAction} selectedTargetIds={targeting.selectedTargetIds} onTarget={targeting.chooseTarget} onHover={(id) => setUiDebug({ hoverTargetId: id })} compact/><PanelTitle>{currentTarget ? "현재 대상" : "내 캐릭터"}</PanelTitle><Inspector entity={currentTarget ?? actor}/><PanelTitle>세션</PanelTitle><div className="session-mini"><span className={snapshot.connectionState === "connected" ? "ok-dot" : "warn-dot"}/>{connectionLabel(snapshot.connectionState)}</div></aside></div>{targeting.selectedAction && <TargetingOverlay />}</div>;
}
...
function DmSceneScreen() {
  ...
  return <div className="screen scene-screen dm-screen"><ScreenHead kicker="DM 세션" title={scene.name} description={`${snapshot.sessionMode === "initiative" ? `${scene.round}라운드 · 현재 턴 ${currentActor.name}` : "자유 진행"} · 선택한 Actor ${selectedActor.name}`} actions={controls}/><div className="scene-layout dm-layout"><aside className="scene-side"><PanelTitle>{snapshot.sessionMode === "initiative" ? "전체 이니셔티브" : "전체 참가자"}</PanelTitle><div className="actor-select-list">{[...scene.entities].sort((a, b) => b.initiative - a.initiative).map((entity) => <button key={entity.id} className={`${entity.id === selectedActor.id ? "selected" : ""} ${entity.id === currentActor.id ? "current" : ""}`} onClick={() => selectDmActor(entity.id)}><span>{snapshot.sessionMode === "initiative" ? entity.initiative : "·"}</span><div><strong>{entity.name}</strong><small>{entity.kind} · HP {entity.hp}/{entity.maxHp}</small></div></button>)}</div><PanelTitle>연결 참가자</PanelTitle>{snapshot.session.participants.map((participant) => <div className="session-mini" key={participant.id}><span className={participant.state === "connected" ? "ok-dot" : "warn-dot"}/>{participant.characterName ?? participant.name} · {participant.state}</div>)}</aside><section className="scene-center"><div className="scene-stage"><div className="formation enemies">{scene.entities.filter((entity) => entity.side === "enemy").map((entity) => <EntityPortrait key={entity.id} entity={entity} selectedAction={targeting.selectedAction} selectedTargetIds={targeting.selectedTargetIds} onTarget={targeting.chooseTarget} onIdle={() => selectDmActor(entity.id)} onHover={(id) => setUiDebug({ hoverTargetId: id })}/>)}</div><div className="scene-context"><strong>{scene.name}</strong><span>선택 Actor와 Current Turn은 서로 다른 상태입니다.</span>{targeting.selectedAction && <em>{targeting.selectedAction.name} · 대상 선택</em>}</div><div className="formation allies">{scene.entities.filter((entity) => entity.side === "ally").map((entity) => <EntityPortrait key={entity.id} entity={entity} selectedAction={targeting.selectedAction} selectedTargetIds={targeting.selectedTargetIds} onTarget={targeting.chooseTarget} onIdle={() => selectDmActor(entity.id)} onHover={(id) => setUiDebug({ hoverTargetId: id })}/>)}</div></div><ActionConsole actor={selectedActor} actions={actions} economy={scene.economyByActor[selectedActor.id]} resources={selectedActor.id === snapshot.activeCharacter.id ? snapshot.activeCharacter.resources.map((resource) => `${resource.label} ${resource.current}/${resource.max}`) : []} filter={filter} setFilter={setFilter} selectedActionId={targeting.selectedActionId} selectedTargetIds={targeting.selectedTargetIds} onAction={targeting.chooseAction} onCancel={targeting.cancel} onCompleteMulti={targeting.completeMulti} sessionMode={snapshot.sessionMode}/></section><aside className="scene-side"><PanelTitle>전투원 Inspector</PanelTitle><Inspector entity={selectedActor}/><PanelTitle>세션 정보</PanelTitle><div className="session-data"><span>RulesProfile <b>dnd.srd-5.2.1</b></span><span>호환 상태 <b>{snapshot.session.compatibility}</b></span><span>세션 콘텐츠 <b>{snapshot.session.sessionContent.length}</b></span></div><PanelTitle>최근 판정</PanelTitle><div className="activity-mini">{snapshot.activity.slice(0, 3).map((entry) => <div key={entry.id}><strong>{entry.title}</strong><span>{entry.summary}</span></div>)}</div></aside></div>{targeting.selectedAction && <TargetingOverlay />}</div>;
}
...
function CombatantsScreen() {
  const { snapshot, previewCombatantImport, activateCombatantImport, clearCombatantImport, instantiateCombatant } = useSimpleVtt();
  const [showImport, setShowImport] = useState(false);
  const [payload, setPayload] = useState('{\n  "id": "combatant.local-bandit",\n  "name": "로컬 산적",\n  "nameEn": "Local Bandit",\n  "ac": 14,\n  "maxHp": 18,\n  "actions": ["단검", "라이트 크로스보우"],\n  "source": "My Encounter Pack",\n  "version": "0.1"\n}');
  if (!snapshot) return null;
  return <div className="screen page-dark"><ScreenHead kicker="컴배턴트" title="컴배턴트 라이브러리" description="재사용 Definition과 세션 Instance를 분리합니다." actions={<button className="primary" onClick={() => setShowImport((value) => !value)}>JSON 가져오기</button>}/>{showImport && <ImportPanel title="Combatant JSON" payload={payload} setPayload={setPayload} preview={snapshot.combatantImport?.validation} onPreview={() => previewCombatantImport(payload)} onActivate={() => activateCombatantImport()} onCancel={() => { setShowImport(false); void clearCombatantImport(); }} canActivate={Boolean(snapshot.combatantImport?.definition && !snapshot.combatantImport.validation.some((item) => item.severity === "blocking"))}>{snapshot.combatantImport?.definition && <div className="review-rows"><div><span>Definition</span><strong>{snapshot.combatantImport.definition.name}</strong></div><div><span>AC / HP</span><strong>{snapshot.combatantImport.definition.ac} / {snapshot.combatantImport.definition.maxHp}</strong></div><div><span>Actions</span><strong>{snapshot.combatantImport.definition.actions.join(", ")}</strong></div></div>}</ImportPanel>}<div className="combatant-grid">{snapshot.combatantDefinitions.map((definition) => <article className="panel-card" key={definition.id}><span className="badge">Definition</span><h2>{definition.name}</h2><small>{definition.nameEn} · {definition.source} · v{definition.version}</small><p>AC {definition.ac} · HP {definition.maxHp}</p><div className="status-chips">{definition.actions.map((action) => <span key={action}>{action}</span>)}</div><button className="primary" onClick={() => instantiateCombatant(definition.id)}>세션에 인스턴스 추가</button></article>)}</div><SectionTitle>현재 Encounter Instances</SectionTitle><div className="combatant-grid compact-cards">{snapshot.scene.entities.filter((entity) => entity.kind === "combatant").map((entity) => <article className="panel-card" key={entity.id}><span className="badge">Instance</span><h3>{entity.name}</h3><p>HP {entity.hp}/{entity.maxHp} · AC {entity.ac}</p><div className="status-chips">{entity.status.map((status) => <span key={status}>{status}</span>)}</div></article>)}</div></div>;
}
...
function CatalogScreen() {
  ...
  return <div className="screen page-dark"><ScreenHead kicker="규칙 콘텐츠" title="규칙 카탈로그" description="한국어가 기본이며 원문명·출처·관계·scope를 상세에서 확인합니다." actions={<button className="primary" onClick={() => setShowImport((value) => !value)}>JSON 가져오기</button>}/>{showImport && <ImportPanel title="Content JSON" payload={payload} setPayload={setPayload} preview={snapshot.contentImport?.validation} onPreview={() => previewContentImport(payload)} onActivate={() => activateContentImport()} onCancel={() => { setShowImport(false); void clearContentImport(); }} canActivate={Boolean(snapshot.contentImport?.entry && !snapshot.contentImport.validation.some((item) => item.severity === "blocking"))} extraActions={<button onClick={() => setPayload('{"id":"subclass.unsafe","category":"subclass","nameKo":"실행 코드 예시","nameEn":"Unsafe","requiresCapability":"arbitrary-code"}')}>지원 불가 예시</button>}>{snapshot.contentImport?.entry && <div className="review-rows"><div><span>콘텐츠</span><strong>{snapshot.contentImport.entry.nameKo} / {snapshot.contentImport.entry.nameEn}</strong></div><div><span>Scope</span><strong>{snapshot.contentImport.entry.scope}</strong></div><div><span>관계</span><strong>{snapshot.contentImport.entry.relationships.map((relationship) => `${relationship.label} → ${relationship.targetName}`).join(", ") || "없음"}</strong></div><div><span>Capabilities</span><strong>{snapshot.contentImport.entry.capabilities.join(", ")}</strong></div></div>}</ImportPanel>}<div className="catalog-layout"><aside className="catalog-categories">{categories.map((id) => <button key={id} className={category === id ? "active" : ""} onClick={() => setCategory(id)}>{id === "all" ? "전체" : categoryLabel(id)}</button>)}</aside><section className="catalog-list"><input placeholder="한국어 / English 검색" value={query} onChange={(event) => setQuery(event.target.value)}/>{filtered.map((entry) => <button key={entry.id} className={selected?.id === entry.id ? "selected" : ""} onClick={() => setSelectedId(entry.id)}><span className="catalog-glyph">{entry.nameKo[0]}</span><div><strong>{entry.nameKo}</strong><small>{entry.nameEn} · {categoryLabel(entry.category)}</small></div><span className={`scope ${entry.scope}`}>{entry.scope}</span></button>)}</section><aside className="catalog-detail">{selected && <><span className="eyebrow accent">{selected.source} · v{selected.version}</span><h2>{selected.nameKo}</h2><small>{selected.nameEn}</small><p>{selected.description}</p><div className="review-rows"><div><span>Scope</span><strong>{selected.scope}</strong></div><div><span>Stable ID</span><strong>{selected.id}</strong></div><div><span>Capabilities</span><strong>{selected.capabilities.join(", ") || "없음"}</strong></div></div>{selected.relationships.length > 0 && <><SectionTitle>관계 Preview</SectionTitle>{selected.relationships.map((relationship) => <div className="relationship-card" key={relationship.targetId}><span>{relationship.label}</span><strong>{selected.nameKo} → {relationship.targetName}</strong><small>{relationship.targetId}</small></div>)}</>}</>}</aside></div></div>;
}
...
function ActivityScreen() {
  ...
  if (!snapshot) return null;
  return <div className="screen page-dark"><ScreenHead kicker="활동 기록" title="활동 기록" description="판정, StateChange, 모드 전환, 아이템, DM 판정과 Undo 관계를 추적합니다." actions={<button onClick={() => undoLastResolution()}>최근 안전 Resolution 되돌리기</button>}/><div className="activity-list">{snapshot.activity.map((entry) => <article key={entry.id} className={`${entry.correction ? "activity-entry correction" : "activity-entry"} ${entry.reversed ? "reversed" : ""}`}><time>{entry.time}</time><div><span className="badge">{entry.actor}</span>{entry.correction && <span className="badge warning">정정/판정</span>}{entry.reversed && <span className="badge warning">되돌려짐</span>}<h3>{entry.title}</h3><p>{entry.summary}</p>{entry.ruling && <p>DM Ruling · {entry.ruling}</p>}{entry.undoOf && <p>Undo 대상 · {entry.undoOf}</p>}<details><summary>Roll / Damage / Provenance / Event 상세</summary>{entry.detail.map((line) => <div key={line}>{line}</div>)}{entry.stateChanges.map((line) => <div key={line}>StateChange · {line}</div>)}<small>ResolutionEvent · {entry.id}</small></details></div></article>)}</div></div>;
}

function SessionScreen() {
  const { snapshot, hostSession, joinSession } = useSimpleVtt();
  const [address, setAddress] = useState(snapshot?.session.address ?? "192.168.0.10:3210");
  if (!snapshot) return null;
  return <div className="screen page-dark"><ScreenHead kicker="세션" title={snapshot.session.name} description="같은 Wi-Fi 또는 Hamachi 주소로 Host/Join하는 로컬 세션 shell입니다."/><div className="session-grid"><article className="panel-card"><span className={`status-text ${snapshot.connectionState}`}>{connectionLabel(snapshot.connectionState)}</span><h2>Host Session</h2><p>이 앱이 세션 권위 Host가 되는 Reference 흐름입니다.</p><button className="primary" onClick={() => hostSession()}>세션 열기</button><div className="review-rows"><div><span>주소</span><strong>{snapshot.session.address}</strong></div><div><span>Role</span><strong>{snapshot.session.role}</strong></div></div></article><article className="panel-card"><h2>Join by IP</h2><Field label="주소"><input value={address} onChange={(event) => setAddress(event.target.value)} placeholder="192.168.0.10:3210" /></Field><button className="primary" onClick={() => joinSession(address)}>참가</button><div className={`compatibility ${snapshot.session.compatibility}`}><strong>{snapshot.session.compatibility}</strong><span>{snapshot.session.compatibilityMessage}</span></div></article><article className="panel-card"><h2>참가자</h2>{snapshot.session.participants.map((participant) => <div className="session-participant" key={participant.id}><span className={participant.state === "connected" ? "ok-dot" : "warn-dot"}/><strong>{participant.characterName ?? participant.name}</strong><small>{participant.state}</small></div>)}</article><article className="panel-card"><h2>세션 콘텐츠</h2>{snapshot.session.sessionContent.map((content) => <div className="session-participant" key={content}><span className="badge">session</span><strong>{content}</strong></div>)}</article></div></div>;
}
...
function SettingsScreen() {
  ...
  return <div className="screen page-dark"><ScreenHead kicker="설정" title="환경 설정" description="Reference 상태 조작은 Ctrl+Shift+D Debug Dock에만 존재합니다."/><div className="settings-card"><SectionTitle>화면</SectionTitle><div className="method-tabs"><button className={theme === "dark" ? "active" : ""} onClick={() => setTheme("dark")}><strong>다크</strong><span>Dark Mode</span></button><button className={theme === "light" ? "active" : ""} onClick={() => setTheme("light")}><strong>라이트</strong><span>Light Mode</span></button></div><SectionTitle>Accent Color</SectionTitle><div className="accent-options">{(["gold", "blue", "green"] as const).map((value) => <button key={value} className={accent === value ? `accent-swatch ${value} selected` : `accent-swatch ${value}`} onClick={() => setAccent(value)}>{value}</button>)}</div><SectionTitle>Motion</SectionTitle><div className="method-tabs"><button className={motion === "normal" ? "active" : ""} onClick={() => setMotion("normal")}><strong>기본</strong><span>짧은 Dice Animation</span></button><button className={motion === "reduced" ? "active" : ""} onClick={() => setMotion("reduced")}><strong>Reduced Motion</strong><span>Animation 최소화</span></button></div><SectionTitle>언어</SectionTitle><div className="derived-callout"><b>한국어 우선</b><span>영문 원문명은 검색·출처 메타데이터에서 보조로 유지합니다.</span></div></div></div>;
}
