...
export function SectionShell({ section, children, aside }: { section: CharacterCreationSection; children: ReactNode; aside?: ReactNode }) {
  return <section id={section.id} className="create-v09-section"><header><div><span className={`create-status-pill ${section.status}`}>{STATUS_LABEL[section.status]}</span><h2>{section.label}</h2><p>{section.description}</p></div>{aside}</header>{section.automaticGrants.length > 0 && <div className="automatic-grants"><span className="eyebrow">AUTOMATIC GRANTS</span>{section.automaticGrants.map((grant) => <span key={grant}>✓ {grant}</span>)}</div>}{children}{section.validation.length > 0 && <div className="validation-list section-validation">{section.validation.map((message, index) => <div className={`validation ${message.severity}`} key={`${message.message}-${index}`}>{message.severity.toUpperCase()} · {message.message}</div>)}</div>}</section>;
}
