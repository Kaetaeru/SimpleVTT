...
function displaySpellIds(ids: string[]) { return ids.map((id) => { const always = id.startsWith("always:"); const raw = always ? id.slice("always:".length) : id; const name = raw.replace(/^dnd\.srd521\.spell\./, "").replaceAll("-", " "); return always ? `항상 준비 · ${name}` : name; }); }
