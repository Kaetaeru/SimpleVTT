...
const ABILITY_CHECK_GROUPS=[
  {label:"근력",ids:["action.ability.str","action.skill.athletics"]},
  {label:"민첩",ids:["action.ability.dex","action.skill.acrobatics","action.skill.sleight-of-hand","action.skill.stealth"]},
  {label:"건강",ids:["action.ability.con"]},
  {label:"지능",ids:["action.ability.int","action.skill.arcana","action.skill.history","action.skill.investigation","action.skill.nature","action.skill.religion"]},
  {label:"지혜",ids:["action.ability.wis","action.skill.animal-handling","action.skill.insight","action.skill.medicine","action.skill.perception","action.skill.survival"]},
  {label:"매력",ids:["action.ability.cha","action.skill.deception","action.skill.intimidation","action.skill.performance","action.skill.persuasion"]},
] as const;
...
export function SessionActionDock({actorId,suspended,targeting,onBeginTargeting,onCancelTargeting,onExecuteTargeting}:{
  ...
  const {snapshot,resolveAction,configureReadyAction,endTurn,refresh}=useSimpleVtt();
  ...
  const abilityActions=useMemo(()=>actions.filter((action)=>action.id.startsWith("action.ability.")||action.id.startsWith("action.skill.")),[actions]);
  ...
  const dockActions=useMemo(()=>[
    ...actions.filter((action)=>!action.id.startsWith("action.ability.")&&!action.id.startsWith("action.skill.")&&!STANDARD_SKILL_GROUPS.some((group)=>action.id.startsWith(`action.standard.${group.id}.`))),
  ...
  return <section className="session-command-center session-reference-command-center" data-action-dock-state={targeting?"target":"hotbar"}>
    <div className="session-command-top">
      <div className="session-command-economy" aria-label="행동 자원">{snapshot.sessionMode==="initiative"&&economy?<><span data-available={economy.action||Boolean(economy.extraActions?.length)||Boolean(economy.extraAttacks?.length)}><i/>행동{economy.extraActions?.length?` +${economy.extraActions.length}`:""}{economy.extraAttacks?.length?` · 추가 공격 ${economy.extraAttacks.length}`:""}</span><span data-available={economy.bonusAction}><i/>보너스</span><span data-available={economy.reaction}><i/>반응</span><span className="session-movement-declare" data-movement={movementDeclaration?.kind??"none"}><i/>이동 {economy.movementMax}피트
        ...
        {approachOpen&&<span className="session-movement-approach-menu" role="menu" aria-label="접근 대상">{approachTargets.length?approachTargets.map((target)=><button type="button" role="menuitem" key={target.id} disabled={movementDisabled} onClick={()=>void declareMovement("approach",target.id)}>{target.name}</button>):<em>접근할 상대가 없습니다.</em>}</span>}
      </span></>:<span className="freeform">자유 진행 · 턴 자원 없음</span>}</div>
    ...
    <div className="session-command-body">
      ...
      <div className="session-hotbar">
        <div className="session-hotbar-tabs" role="tablist" aria-label="핫바 분류"><button type="button" className={`session-hotbar-menu-trigger ${actionMenu==="ability"?"active":""}`} aria-expanded={actionMenu==="ability"} onClick={()=>{setStandardSkillPicker(null);setActionMenu((open)=>open==="ability"?null:"ability");}}>능력 판정</button>{role==="dm"&&<button type="button" className={`session-hotbar-menu-trigger session-hotbar-hidden-roll ${snapshot.resolutionVisibility?.armed?"active":""}`} aria-label="비공개 굴림" aria-pressed={Boolean(snapshot.resolutionVisibility?.armed)} title="다음 판정의 주사위·결과·대상을 플레이어에게 숨깁니다" onClick={()=>void setNextResolutionVisibility(snapshot.resolutionVisibility?.armed?null:{hidden:["roll","targets"]})}>비공개 굴림</button>}{HOTBAR_PAGES.map((entry)=><button type="button" role="tab" key={entry.id} aria-selected={page===entry.id} className={page===entry.id?"active":""} onClick={()=>{setPage(entry.id);setActionMenu(null);setStandardSkillPicker(null);onCancelTargeting();}}>{entry.label}</button>)}<span className="session-hotbar-row-control" aria-label="핫바 줄 수"><button type="button" aria-label="핫바 줄 줄이기" disabled={rows===2} onClick={()=>changeRows(rows-1)}>−</button><b>{rows}줄</b><button type="button" aria-label="핫바 줄 늘리기" disabled={rows===4} onClick={()=>changeRows(rows+1)}>＋</button></span></div>
    ...
    {actionMenu==="ability"&&<aside className="session-action-library ability" aria-label="능력 판정 선택"><header><div><span>D20 TEST</span><strong>능력 판정</strong><small>능력 또는 기술을 선택하면 캐릭터 수정치와 숙련을 적용해 굴립니다.</small></div><button type="button" aria-label="능력 판정 닫기" onClick={()=>setActionMenu(null)}>×</button></header><div className="session-ability-check-groups">{ABILITY_CHECK_GROUPS.map((group)=><section key={group.label}><strong>{group.label}</strong><div>{group.ids.map((id)=>abilityActions.find((action)=>action.id===id)).filter((action):action is ActionVm=>Boolean(action)).map(menuActionButton)}</div></section>)}</div></aside>}
