...
function supportedInterceptor(value:Obj) {
  return value.operation==="recalculate"&&(
    (value.timing==="d20.outcome-determined"&&value.slot==="d20.roll")
 ...
 * Unsupported interceptor families remain data-only and are ignored here rather than gaining
 * content-name branches or an implicit second execution engine.
 */
export function lowerCommonPlayReactionDefinition(definition:CommonPlayDefinitionIR):CommonPlayReactionDefinition|undefined {
  const selected=(definition.interceptors??[])
    .map((candidate,index)=>({candidate,index}))
    .filter(({candidate})=>supportedInterceptor(candidate));
  if(!selected.length) return undefined;
  const interceptors=selected.map(({candidate,index})=>
