...
function operationEntryPointProjection(entryPoint:CommonPlayEntryPointIR) {
  ...
  projected.targeting={...structuredClone(targeting),from:"targets",min:Number(targeting.min),max:Number(targeting.max)};
  return projected;
}
...
export function lowerCommonPlay(
  ...
  if([...operationKinds].some((kind)=>artifactOperationKinds.has(String(kind)))) {
    ...
    if(portableOperations.length) {
      if(![...artifactKinds].every((kind)=>kind==="stored-invocation"||kind==="object"||kind==="link"||kind==="actor"||kind==="form"||kind==="exposure"||kind==="environment"||kind==="project")) {
    ...
    if([...artifactKinds].every((kind)=>kind==="stored-invocation"||kind==="object"||kind==="link"||kind==="actor"||kind==="form"||kind==="exposure"||kind==="environment"||kind==="project")) return {
