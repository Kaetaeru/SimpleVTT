...
export interface CampaignsState {
  ...
  cast: (caster: ActorRef, spellId: string, targets: ActorRef[], method?: CastMethod, overrides?: AttackOverrides, readied?: boolean, reaction?: string, variant?: string, metamagic?: string[]) => void;
  ...
  /** R63 (D198): answer the window a hit opened — what to add, the facts confirmed, the smite spell and its slot. */
  hitChoice: (messageId: string, choices: string[], facts?: string[], spellSmite?: { spellId: string; slot: number }) => void;
...
export function CampaignsProvider({ children }: { children: ReactNode }) {
  ...
  const cast = useCallback((caster: ActorRef, spellId: string, targets: ActorRef[], method?: CastMethod, overrides?: AttackOverrides, readied?: boolean, reaction?: string, variant?: string, metamagic?: string[]) => send({ type: "act.cast", caster, spellId, targets, method, overrides, readied, reaction, ...(variant ? { variant } : {}), ...(metamagic?.length ? { metamagic } : {}) }), [send]);
  ...
  const hitChoice = useCallback((messageId: string, choices: string[], facts?: string[], spellSmite?: { spellId: string; slot: number }) => send({ type: "act.onhit", messageId, choices, ...(facts?.length ? { facts } : {}), ...(spellSmite ? { spellSmite } : {}) }), [send]);
