  ...
  return {
    ...
    effects: Array.isArray(value.effects)
      ? value.effects.filter((entry): entry is ActiveEffect => isObject(entry) && typeof entry.key === "string" && typeof entry.name === "string" && (entry.source === "feature" || entry.source === "spell") && typeof entry.duration === "string")
        .map((entry) => ({ key: entry.key, name: entry.name, source: entry.source, duration: entry.duration, concentration: entry.concentration === true, rounds: Number.isInteger(entry.rounds) ? entry.rounds : undefined, elapsed: Number.isInteger(entry.elapsed) ? entry.elapsed : 0, startedAt: typeof entry.startedAt === "string" ? entry.startedAt : new Date().toISOString(), ...(Number.isInteger(entry.level) ? { level: entry.level } : {}), ...(typeof entry.from === "string" ? { from: entry.from } : {}), ...(entry.fromConcentration === true ? { fromConcentration: true } : {}), ...(entry.anchor && typeof entry.anchor === "object" ? { anchor: entry.anchor } : {}), ...(Array.isArray(entry.conditions) ? { conditions: entry.conditions.map(String) } : {}), ...(entry.bearer === true ? { bearer: true } : {}), ...(typeof entry.variant === "string" ? { variant: entry.variant } : {}), ...(entry.rescue && typeof entry.rescue === "object" && typeof (entry.rescue as { dice?: unknown }).dice === "string" ? { rescue: { dice: (entry.rescue as { dice: string }).dice } } : {}) }))
      : [],
