...
import type { CharacterSource, DerivedAttack, DerivedCharacter, DerivedSkill, DerivedSpellcasting } from "./types";
...
function finalize(ledger: Ledger): DerivedCharacter {
  ...
  if (ledger.hpPerLevelBonus) { hpMax += ledger.hpPerLevelBonus * level; hpBreakdown.push(`레벨당 +${ledger.hpPerLevelBonus} × ${level} = ${ledger.hpPerLevelBonus * level}`); }
  if (ledger.hpFlatBonus) { hpMax += ledger.hpFlatBonus; hpBreakdown.push(`추가 +${ledger.hpFlatBonus}`); }
  ...
  const acCandidates: Array<{ value: number; source: string; breakdown: string[] }> = [];
  const dex = mod("dex");
  ...
  if (armorView?.armor) {
    ...
    if (shieldBonus) lines.push(`방패 +${shieldBonus}`);
    if (defenseStyle) lines.push("전투 방식(방어) +1");
    acCandidates.push({ value, source: armorView.name, breakdown: lines });
  } else {
    acCandidates.push({ value: 10 + dex + shieldBonus, source: "방어구 없음", breakdown: [`기본 10`, `민첩 ${dex}`, ...(shieldBonus ? [`방패 +${shieldBonus}`] : [])] });
    if (ledger.flags.has("unarmored-defense:barbarian")) acCandidates.push({ value: 10 + dex + mod("con") + shieldBonus, source: "비무장 방어 (바바리안)", breakdown: ["기본 10", `민첩 ${dex}`, `건강 ${mod("con")}`, ...(shieldBonus ? [`방패 +${shieldBonus}`] : [])] });
    if (ledger.flags.has("unarmored-defense:monk") && !shieldView) acCandidates.push({ value: 10 + dex + mod("wis"), source: "비무장 방어 (몽크)", breakdown: ["기본 10", `민첩 ${dex}`, `지혜 ${mod("wis")}`] });
    if (ledger.features.some((feature) => feature.id.endsWith("draconic-sorcery.draconic-resilience"))) acCandidates.push({ value: 10 + dex + mod("cha") + shieldBonus, source: "용의 회복력", breakdown: ["기본 10", `민첩 ${dex}`, `매력 ${mod("cha")}`, ...(shieldBonus ? [`방패 +${shieldBonus}`] : [])] });
  }
  ...
  if (ledger.flags.has("fast-movement") && !heavyArmorWorn) walk += 10;
  if (ledger.flags.has("unarmored-movement") && !armorView && !shieldView) {
    ...
    walk += numericColumn(row?.columns[COLUMN.unarmoredMovement]);
  }
  if (strengthShort) walk -= 10;
  const speed: DerivedCharacter["speed"] = { walk };
  ...
  for (const key of ABILITY_KEYS) {
    ...
    saves[key] = { proficient, bonus: mod(key) + (proficient ? pb : 0) };
  }
  ...
  const skills: DerivedSkill[] = Object.entries(catalog.skills).map(([id, name]) => {
    ...
    return { id, name, ability, proficient, expertise, bonus };
  ...
  return {
    ...
    classes,
    abilities,
    hp: { max: hpMax, breakdown: hpBreakdown },
    ac,
    speed,
...
function weaponAttack(ledger: Ledger, view: ItemView, abilities: DerivedCharacter["abilities"], pb: number): DerivedAttack {
  ...
  return {
    id: `attack.${view.id}`, name: view.name, itemId: view.id, ability, attackBonus,
    damage: versatile ? `${weapon.damage} (양손 ${versatile})` : weapon.damage, damageBonus: abilities[ability].modifier, damageType: damageTypeKo(weapon.damageType),
...
function unarmedStrike(ledger: Ledger, abilities: DerivedCharacter["abilities"], pb: number): DerivedAttack {
  ...
  return { id: "attack.unarmed-strike", name: "비무장 공격", ability, attackBonus: abilities[ability].modifier + pb, damage, damageBonus: abilities[ability].modifier, damageType: "타격", properties: monk ? ["무예"] : [], masteryActive: false };
}
