    ...
    for (const item of contract.interceptors) {
      ...
      for (const operation of item.operations) {
        ...
        if (operation.kind === "property.modify" && operation.property === "ac.bonus") {
          ...
          const noSave = operation.params?.save === "none";
          if (operation.dice && (noSave || Number.isFinite(dc))) offer.redirect = { formula: `${operation.dice}${Number.isFinite(flat) && flat ? `+${flat}` : ""}`, damageType: String(operation.params?.damageType ?? "역장"), ...(noSave ? {} : { save: { ability: String(operation.params?.ability ?? "dex"), dc } }) };
