...
/** R63 (D198): one thing the attacker may add after a hit — a built-in rider (`savage`), a smite spell (`spell:<id>`) or a contract's rule key. */
export interface HitOffer {
  ...
  facts?: Array<{ id: string; question: string; /** H2 (D239): the table computes this fact; it is never a checkbox. */ auto?: string; /** V4h (D270): a computed fact that falls back to a checkbox when it is false (암습). */ orAsk?: boolean }>;
