...
export class TableHost {
      ...
      case "act.contract": {
        ...
        if (targets.length && (party.tempHp || party.heal || party.grants.length)) {
          ...
          for (const target of capped) {
            ...
            if (party.heal) { const amount = rollGuard(party.heal, party.healMaximized ? () => 0.999999 : this.options.random ?? Math.random); this.healActor(target, amount); given.push(`회복 ${amount}${party.healMaximized ? " (최대값)" : ""}`); }
  ...
  private consumeOnUse(entryId: string, on: "attack" | "attacked" | "cast") {
    ...
    const keys = [...(bearerRolls(live.runtime.effects, false).consumable ?? []).filter((item) => item.on === on).map((item) => item.key), ...(live.runtime.effects ?? []).filter((effect) => (effect.consumeOn === "attack-or-cast" ? on === "attack" || on === "cast" : effect.consumeOn === on)).map((effect) => effect.key)];
  ...
  private riderSave(attacker: { entry: JournalEntry; token?: Token; page?: Page }, target: { entry: JournalEntry; token?: Token; page?: Page }, rule: { label: string; ability: string; dc: number; condition: string; also?: string[]; duration?: ConditionDuration; repeatSave?: "turn-end"; successMark?: TargetMark }, waits: boolean, by: string) {
    const live = this.resolveActor(this.refOf(target)) ?? target;
    const combatant = this.combatantOf(live);
    const stats = this.statsOf(live);
    ...
    const exec: SpellExec = { spellId: `feature:${rule.label}`, baseLevel: 0, castingEconomy: "action", targeting: { kind: "creature", minTargets: 1, maxTargets: 1 }, primary: { kind: "save-effect", saveAbility: rule.ability, duration }, effects: [rule.condition, ...(rule.also ?? [])].map((conditionId) => ({ conditionId, trigger: "failed-save" as const, duration })), ...(rule.repeatSave ? { repeatSave: rule.repeatSave } : {}) };
