...
export function ApprovalLayer() {
  ...
  const bodyOf = (message: ChatMessage) => {
    ...
    return prompt.kind === "counterspell" ? <div className="cl-approval-body">🚫 <strong>{prompt.mover.name}</strong>이(가) {prompt.spell?.name}{prompt.spell ? ` (${prompt.spell.level}레벨)` : ""}을(를) 시전하려 합니다.<br />주문 차단을 하시겠습니까?</div>
      ...
      : prompt.kind === "trigger" ? <div className="cl-approval-body">✨ <strong>{prompt.reactor.name}</strong>: {prompt.trigger?.event === "initiative" ? "이니셔티브를 굴렸습니다" : prompt.trigger?.event === "kill" ? "적을 쓰러뜨렸습니다" : "짧은 휴식을 마쳤습니다"}.<br />지금 쓸 수 있는 특성이 있습니다.</div>
