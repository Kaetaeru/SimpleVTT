...
/** Toasts for chat that arrived after mount; each fades after a few seconds, at most four at once. */
export function ToastLayer() {
  ...
  useEffect(() => {
    ...
    const next = fresh.map((message) => ({ message, toast: toastText(message) })).filter((item): item is { message: ChatMessage; toast: { text: string; tone: "info" | "good" | "bad" } } => Boolean(item.toast)).map((item) => ({ id: item.message.id, ...item.toast }));
    if (!next.length) return;
    setToasts((list) => [...list, ...next].slice(-4));
...
/** What waits for this viewer's answer: opportunity prompts addressed to them, and (DM) results waiting for 적용 (D90). */
export function ApprovalLayer() {
  ...
  const items = [
    ...prompts.map((message) => ({ key: message.id, node: <PromptItem message={message} /> })),
    ...waiting.map((message) => ({ key: message.id, node: (
      <div className="cl-approval">
        <div className="cl-small"><Pill tone="accent">DM 확인</Pill> {message.action!.attacker.name} → {message.action!.target.name}: {message.action!.attack.name} — {message.action!.outcome === "crit" ? "치명타" : message.action!.outcome === "hit" ? "적중" : "빗나감"}{message.action!.damageTotal ? ` · 피해 ${message.action!.damageTotal}` : ""}</div>
        <div className="cl-row" style={{ gap: 4 }}><button type="button" className="cl-btn small primary" onClick={() => c.confirmAction(message.id)}>적용</button><button type="button" className="cl-btn small" onClick={() => c.adjustAction(message.id, { outcome: "miss" })}>빗나감으로</button><button type="button" className="cl-btn small quiet" onClick={() => c.undoAction(message.id)}>취소</button></div>
      </div>
    ) })),
  ];
  return <aside className="cl-approvals" aria-label="승인">{items.map((item) => <div key={item.key}>{item.node}</div>)}</aside>;
}

function PromptItem({ message }: { message: ChatMessage }) {
  const choices = PromptChoices({ message, compact: true });
  if (!choices) return null;
  return (
    <div className="cl-approval">
      <div className="cl-small">🏃 {message.prompt!.mover.name}이(가) <strong>{message.prompt!.reactor.name}</strong>에게서 벗어납니다 — 기회 공격?</div>
      {choices}
    </div>
  );
}
