...
function NpcGmFields({ entry }: { entry: JournalNpc }) {
  ...
  return (
    <div className="cl-card" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      {/* R27 (D146): handing an NPC ally to a player used to need three switches across two panels, because this
          panel had no 고칠 수 있는 사람 field at all and the token's controller select was the only way in. */}
      <div className="cl-field"><label>맡길 사람 (조종)</label><div className="cl-row" style={{ gap: 6 }}>{players.length ? players.map((player) => <label key={player.userId} className="cl-row cl-small" style={{ gap: 4 }}><input type="checkbox" checked={entry.canEdit !== "all" && entry.canEdit.includes(player.userId)} onChange={(event) => { const list = entry.canEdit === "all" ? [] : entry.canEdit; const next = event.target.checked ? [...new Set([...list, player.userId])] : list.filter((id) => id !== player.userId); const view = entry.canView === "all" ? "all" as const : [...new Set([...entry.canView, ...(event.target.checked ? [player.userId] : [])])]; put({ canEdit: next, canView: view }); }} />{player.displayName}</label>) : <span className="cl-quiet cl-small">아직 참가자가 없습니다</span>}</div><span className="cl-quiet cl-small">맡은 사람은 이 NPC를 판에서 움직이고 그 행동·공격을 쓸 수 있습니다 (볼 수 있는 사람에도 함께 들어갑니다).</span></div>
