    ...
    <div className="cl-page">
      ...
      <div className="cl-grid-2">
        <div className="cl-card">
          ...
          <label className="cl-row cl-small" style={{ gap: 6 }}>응답 없는 창의 기본값 적용까지 <input className="cl-input" type="number" min={0} style={{ width: 70 }} aria-label="응답 제한 시간 (초)" value={campaign.settings.promptTimeoutSeconds ?? 90} onChange={(event) => update({ settings: { ...campaign.settings, promptTimeoutSeconds: Math.max(0, Number(event.target.value) || 0) } })} />초 (0 = 무제한)</label>
          {/* R29 (D154): the death save is the player's roll by default; turning this off has the host roll it. */}
          <label className="cl-row cl-small" style={{ gap: 6 }}><input type="checkbox" checked={campaign.settings.playersRollDeathSaves !== false} onChange={(event) => update({ settings: { ...campaign.settings, playersRollDeathSaves: event.target.checked } })} /> 죽음 내성은 그 인물의 플레이어가 굴린다 (끄면 호스트가 턴 시작에 바로 굴립니다)</label>
