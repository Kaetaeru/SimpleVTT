    ...
    <div className="cl-page">
      ...
      <div className="cl-grid-2">
        <div className="cl-card">
          ...
          <div className="cl-field"><label htmlFor="cl-table-mode">테이블 방식</label><select id="cl-table-mode" className="cl-select" value={campaign.settings.tableMode ?? "totm"} onChange={(event) => update({ settings: { ...campaign.settings, tableMode: event.target.value as "totm" | "grid" } })}><option value="totm">장면 (Theatre of the Mind) — 위치·거리 없음, 엄폐·유리는 DM이 판단</option><option value="grid">격자 지도 — 토큰 위치와 사거리를 추적</option></select><span className="cl-quiet cl-small">장면에서도 HP·상태·집중·반응은 계속 추적됩니다. "벗어남" 버튼이 기회 공격을 물어봅니다.</span></div>
