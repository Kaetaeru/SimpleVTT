...
export function checkExecutionCompositionBoundary(repoRoot) {
  const compositionPath=join(repoRoot,"src","app","offlineRuntimeAdapters.ts");
  const baselinePath=join(repoRoot,".agents","EXECUTION_COMPOSITION_BASELINE.json");
  const errors=[];
...
if (resolve(process.argv[1] ?? "")===self) {
  ...
  const result=checkExecutionCompositionBoundary(repoRoot);
  if (!result.ok) {
    console.error("Execution composition boundary drift detected:");
    ...
    console.log(`Execution composition boundary OK: ${result.actual.length} classified import(s).`);
