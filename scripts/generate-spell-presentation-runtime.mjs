...
const outputPath = join(root, "src", "generated", "spellPresentationPayload.generated.ts");
...
for (const spell of catalog.spells) {
  ...
  if (!Number.isInteger(spell.level) || spell.level < 0 || spell.level > 9) throw new Error(`${spell.id}: invalid level ${spell.level}`);
...
const source = [
  "// Generated by scripts/generate-spell-presentation-runtime.mjs. Do not edit.",
  `export const SPELL_PRESENTATION_PAYLOAD_BASE64 = ${JSON.stringify(encoded)} as const;`,
  `export const SPELL_PRESENTATION_PAYLOAD_SHA256 = ${JSON.stringify(digest)} as const;`,
  "",
].join("\n");
writeFileSync(outputPath, source, "utf8");
console.log(`Generated compressed SRD spell presentation runtime payload: ${catalog.count} spells, ${compressed.length} bytes, sha256 ${digest.slice(0, 12)}...`);
