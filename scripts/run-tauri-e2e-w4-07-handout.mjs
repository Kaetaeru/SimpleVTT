...
async function run(){
  ...
  const ledgerBeforeWithdraw=await ledgerCursor(host);await click(host.browser,exactButton("공유 철회"),"Handout 공유 철회");const p1Withdraw=await waitForHandout(p1,null);const p2Withdraw=await waitForHandout(p2,null);await waitAbsent(p1,".session-handout-viewer","P1 withdrawn handout viewer");await waitAbsent(p2,".session-handout-viewer","P2 withdrawn handout viewer");assert.equal(await ledgerCursor(host),ledgerBeforeWithdraw,"G04 handout withdraw changed mechanics ledger");await disconnect(p1);await joinClient(p1,port);const withdrawnReconnect=await waitForHandout(p1,null);await waitAbsent(p1,".session-handout-viewer","G05 withdrawn reconnect viewer");
