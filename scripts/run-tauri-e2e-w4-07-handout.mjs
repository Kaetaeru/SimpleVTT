...
async function disconnect(client){const result=await client.browser.executeAsync((done)=>{import("/src/app/mockAdapter.ts").then(async({mockAdapter})=>{await mockAdapter.stopSession();done({ok:true});}).catch((error)=>done({error:String(error?.stack??error)}));});assert.ok(!result.error,result.error);await client.browser.waitUntil(async()=>!(await client.browser.$("body").getText()).includes("클라이언트 · 플레이어"),{timeout:20_000});}
...
async function run(){
  ...
  await click(host.browser,"//header[contains(@class,'session-reference-play-chrome')]//button[normalize-space(.)='세션']","Host 세션 도구");await click(host.browser,exactButton("이미지 보여주기"),"Host Handout 열기");await host.browser.$("//*[@data-testid='session-dm-handout']").waitForDisplayed({timeout:15_000});
  const handoutPath=path.join(runRoot,"g04-handout.png");await writeFile(handoutPath,Buffer.from("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Y9Z9xkAAAAASUVORK5CYII=","base64"));const fileInput=await host.browser.$("//*[@data-testid='session-dm-handout']//input[@type='file']");await fileInput.setValue(handoutPath);await waitDisplayed(host,".session-handout-preview","Host private handout preview");
