...
async function run(){
  ...
  const handoutPath=path.join(runRoot,"g04-handout.png");await writeFile(handoutPath,Buffer.from("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Y9Z9xkAAAAASUVORK5CYII=","base64"));const fileInput=await host.browser.$("//aside[@aria-label='DM Handout 도구']//input[@type='file']");await fileInput.setValue(handoutPath);await waitDisplayed(host,".session-handout-preview","Host private handout preview");
