...
async function materializeHostCustomJson(host,p1,p2,p1CharacterId){
  const result=await host.browser.executeAsync((characterId,done)=>{
    (async()=>{
      ...
      let snapshot=await mockAdapter.getSnapshot();let currentCampaign=snapshot.campaigns?.find((entry)=>entry.campaignId===campaign.campaignId);const item=currentCampaign?.dmLibrary.entries.find((entry)=>entry.entryId==="w4.g03.item");const npc=currentCampaign?.dmLibrary.entries.find((entry)=>entry.entryId==="w4.g03.npc");const image=currentCampaign?.dmLibrary.entries.find((entry)=>entry.entryId==="w4.g03.image");
      snapshot=await mockAdapter.grantCampaignDmLibraryItem(campaign.campaignId,"w4.g03.item",{kind:"character",actorId:characterId},1);const granted=snapshot.sessionCharacterInventories?.[characterId]?.items.find((entry)=>entry.definitionId==="local.w4.g03.storm-crown");
      ...
      return {item:{attunementRequired:item?.itemTemplate?.attunementRequired,charges:item?.itemTemplate?.charges,grantedActionIds:item?.itemTemplate?.grantedActionIds,provenance:item?.itemTemplate?.provenance},npc:{source:npc?.npcDefinition?.source,version:npc?.npcDefinition?.version,maxHp:npc?.npcDefinition?.maxHp},image:image?.imageAsset,granted:granted?{definitionId:granted.definitionId,provenance:granted.provenance}:null,actor:actor?{id:actor.id,name:actor.name,kind:actor.kind,side:actor.side,hp:actor.hp,maxHp:actor.maxHp}:null,rejected};
    })().then(done).catch((error)=>done({error:String(error?.stack??error)}));
  },p1CharacterId);
  assert.ok(!result.error,result.error);assert.deepEqual(result.item.charges,{current:3,max:5});assert.equal(result.item.attunementRequired,true);assert.deepEqual(result.item.grantedActionIds,["action.storm-bolt"]);assert.deepEqual(result.item.provenance,["W4-G03-PROVENANCE"]);assert.deepEqual(result.granted,{definitionId:"local.w4.g03.storm-crown",provenance:["W4-G03-PROVENANCE"]});
...
async function runScenario(){
  ...
  const customJson=await materializeHostCustomJson(host,p1,p2,p1Character.id);
