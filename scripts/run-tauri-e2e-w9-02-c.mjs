...
async function runScenario(){
  ...
  try{
    ...
    const slotOf=(state)=>state.slots.find((l)=>l.level===1)??state.resources.find((r)=>/spell-slot-1/.test(r.id));const slotBefore=slotOf(await peerState(p2));
    ...
    const slotAfter=slotOf(await peerState(p2));
    ...
    const slotAfterCantrip=slotOf(await peerState(p2));assert.equal(slotAfterCantrip.current,slotAfter.current,"a cantrip must not spend a slot");
    ...
    hostRes=await waitHostResolutionFor(host,cleric2.id);const helpDone=await hostAdvanceToComplete(host,hostRes.resolution.id);assert.equal(helpDone.resolution?.stage,"complete",JSON.stringify(helpDone.resolution));
    const helpCursor=(await peerState(host)).cursor;for(const p of [p1,p2])await p.browser.waitUntil(async()=>(await peerState(p)).cursor>=helpCursor,{timeout:20_000,timeoutMsg:`${p.label} did not reach the Host cursor after Help`});
    for(const p of peers)await p.browser.waitUntil(async()=>(entity(await peerState(p),fighter1.id)?.status??[]).some((status)=>/도움/.test(status)),{timeout:15_000,timeoutMsg:`${p.label} did not project the Helped status on P1`});
  ...
  }catch(error){await evidenceAll(peers,"w9-02c-failure");const diag={};for(const p of peers)diag[p.label]=await bounded(peerState(p).catch((e)=>({error:String(e)})),8_000,"state");diag.hostLedger=await bounded(hostCall(host,HOST_LEDGER_DIAG).catch((e)=>({error:String(e)})),8_000,"ledger");await writeFile(path.join(artifactRoot,"w9-02c-failure-state.json"),`${JSON.stringify({scenarios,diag},null,2)}\n`,"utf8").catch(()=>undefined);throw new Error(`${error instanceof Error?error.message:String(error)}; passed=${Object.keys(scenarios).join(",")} states=${JSON.stringify(diag).slice(0,3000)}`);}
