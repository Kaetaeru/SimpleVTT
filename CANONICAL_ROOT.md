...
gate: npm run gate                          # gate:client(타입·경계·단위) + gate:e2e(브라우저 13개)
...
current_spec: docs/design/v3/ROLL20_TABLE_SPEC.md   # 요소 전수 + 결정 기록 D1~D115
...
- `client/session/` — 호스트 권위(`TableHost`)와 거울(`TableClient`), 프로토콜(현재 v18). 판정은 전부 호스트에서 난다.
- `tests/client/*.test.ts` — 단위 136개. `scripts/capture-client-*.mjs` — 브라우저 E2E 13개, 증거는
...
3. `docs/CURRENT.md`와 `docs/design/v3/ROLL20_TABLE_SPEC.md`의 결정 기록(D1~D115).
